...
#ifndef QSBOOST_MULTI_INDEX_SEQUENCED_INDEX_HPP
#define QSBOOST_MULTI_INDEX_SEQUENCED_INDEX_HPP
...
#include <qsboost/multi_index/detail/bidir_node_iterator.hpp>
#include <qsboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <qsboost/multi_index/detail/index_node_base.hpp>
#include <qsboost/multi_index/detail/safe_mode.hpp>
#include <qsboost/multi_index/detail/scope_guard.hpp>
#include <qsboost/multi_index/detail/seq_index_node.hpp>
#include <qsboost/multi_index/detail/seq_index_ops.hpp>
#include <qsboost/multi_index/detail/vartempl_support.hpp>
#include <qsboost/multi_index/sequenced_index_fwd.hpp>
#include <qsboost/tuple/tuple.hpp>
...
#define QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT_OF(x)                    \
...
#define QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT                          \
  QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT_OF(*this)
#else
#define QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT_OF(x)
#define QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT
#endif
...
class sequenced_index:
  QSBOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS SuperMeta::type
...
public:
  ...
  typedef typename
    qsboost::reverse_iterator<iterator>                 reverse_iterator;
  typedef typename
    qsboost::reverse_iterator<const_iterator>           const_reverse_iterator;
...
public:
  ...
  {
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    clear();
  ...
  iterator  begin()QSBOOST_NOEXCEPT
    {return make_iterator(node_type::from_impl(header()->next()));}
  const_iterator begin()const QSBOOST_NOEXCEPT
    {return make_iterator(node_type::from_impl(header()->next()));}
  iterator
    end()QSBOOST_NOEXCEPT{return make_iterator(header());}
  const_iterator
    end()const QSBOOST_NOEXCEPT{return make_iterator(header());}
  reverse_iterator
    rbegin()QSBOOST_NOEXCEPT{return qsboost::make_reverse_iterator(end());}
  const_reverse_iterator
    rbegin()const QSBOOST_NOEXCEPT{return qsboost::make_reverse_iterator(end());}
  reverse_iterator
    rend()QSBOOST_NOEXCEPT{return qsboost::make_reverse_iterator(begin());}
  const_reverse_iterator
    rend()const QSBOOST_NOEXCEPT{return qsboost::make_reverse_iterator(begin());}
  const_iterator
    cbegin()const QSBOOST_NOEXCEPT{return begin();}
  const_iterator
    cend()const QSBOOST_NOEXCEPT{return end();}
  const_reverse_iterator
    crbegin()const QSBOOST_NOEXCEPT{return rbegin();}
  const_reverse_iterator
    crend()const QSBOOST_NOEXCEPT{return rend();}
  ...
  bool      empty()const QSBOOST_NOEXCEPT{return this->final_empty_();}
  size_type size()const QSBOOST_NOEXCEPT{return this->final_size_();}
  size_type max_size()const QSBOOST_NOEXCEPT{return this->final_max_size_();}
  ...
  void resize(size_type n)
  {
    ...
    if(n>size()){
      for(size_type m=n-size();m--;)
        this->final_emplace_(QSBOOST_MULTI_INDEX_NULL_PARAM_PACK);
    }
  ...
  QSBOOST_MULTI_INDEX_OVERLOADS_TO_VARTEMPL(
    emplace_return_type,emplace_front,emplace_front_impl)

  std::pair<iterator,bool> push_front(const value_type& x)
                             {return insert(begin(),x);}
  std::pair<iterator,bool> push_front(QSBOOST_RV_REF(value_type) x)
                             {return insert(begin(),qsboost::move(x));}
  ...
  std::pair<iterator,bool> push_back(const value_type& x)
                             {return insert(end(),x);}
  std::pair<iterator,bool> push_back(QSBOOST_RV_REF(value_type) x)
                             {return insert(end(),qsboost::move(x));}
  void                     pop_back(){erase(--end());}

  QSBOOST_MULTI_INDEX_OVERLOADS_TO_VARTEMPL_EXTRA_ARG(
    emplace_return_type,emplace,emplace_impl,iterator,position)

  std::pair<iterator,bool> insert(iterator position,const value_type& x)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
  ...
  std::pair<iterator,bool> insert(iterator position,QSBOOST_RV_REF(value_type) x)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
  ...
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
  ...
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(first);
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(last);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(first,*this);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(last,*this);
    QSBOOST_MULTI_INDEX_CHECK_VALID_RANGE(first,last);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    while(first!=last){
      first=erase(first);
    }
  ...
  bool replace(iterator position,QSBOOST_RV_REF(value_type) x)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
  ...
  {
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT_OF(x);
  ...
  void splice(iterator position,sequenced_index<SuperMeta,TagList>& x)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_CHECK_DIFFERENT_CONTAINER(*this,x);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
  ...
  void splice(iterator position,sequenced_index<SuperMeta,TagList>& x,iterator i)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(i);
    QSBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(i);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(i,x);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    if(&x==this){
    ...
    iterator first,iterator last)
  {
    ...
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(first);
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(last);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(first,x);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(last,x);
    QSBOOST_MULTI_INDEX_CHECK_VALID_RANGE(first,last);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    if(&x==this){
      QSBOOST_MULTI_INDEX_CHECK_OUTSIDE_RANGE(position,first,last);
  ...
  void remove(value_param_type value)
  {
    sequenced_index_remove(
      *this,
      ::qsboost::bind(std::equal_to<value_type>(),::qsboost::arg<1>(),value));
  }
  ...
  void relocate(iterator position,iterator i)
  {
    ...
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(i);
    QSBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(i);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(i,*this);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    if(position!=i)relink(position.get_node(),i.get_node());
  }
...
QSBOOST_MULTI_INDEX_PROTECTED_IF_MEMBER_TEMPLATE_FRIENDS:
    ...
    Archive& ar,const unsigned int version,const index_loader_type& lm)
  {
    lm.load(
      ::qsboost::bind(
        &sequenced_index::rearranger,this,::qsboost::arg<1>(),::qsboost::arg<2>()),
      ar,version);
...
private:
  ...
  {
    return emplace_impl(begin(),QSBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
  }
  ...
  std::pair<iterator,bool> emplace_back_impl(
    QSBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    return emplace_impl(end(),QSBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
  }

  template<QSBOOST_MULTI_INDEX_TEMPLATE_PARAM_PACK>
  std::pair<iterator,bool> emplace_impl(
    iterator position,QSBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT;
    std::pair<final_node_type*,bool> p=
      this->final_emplace_(QSBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
    if(p.second&&position.get_node()!=header()){
      relink(position.get_node(),p.first);
    }
    return std::pair<iterator,bool>(make_iterator(p.first),p.second);
...
struct sequenced
{
  QSBOOST_STATIC_ASSERT(detail::is_tag<TagList>::value);
...
template<typename SuperMeta,typename TagList>
inline qsboost::mpl::true_* qsboost_foreach_is_noncopyable(
  qsboost::multi_index::detail::sequenced_index<SuperMeta,TagList>*&,
  qsboost_foreach_argument_dependent_lookup_hack)
{
  return 0;
}

#undef QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT
#undef QSBOOST_MULTI_INDEX_SEQ_INDEX_CHECK_INVARIANT_OF
