...
#ifndef QSBOOST_MULTI_INDEX_DETAIL_COPY_MAP_HPP
#define QSBOOST_MULTI_INDEX_DETAIL_COPY_MAP_HPP
...
#include <qsboost/multi_index/detail/auto_space.hpp>
#include <qsboost/multi_index/detail/raw_ptr.hpp>
...
public:
  ...
  ~copy_map()
  {
    if(!released){
      for(std::size_t i=0;i<n;++i){
        qsboost::detail::allocator::destroy(&(spc.data()+i)->second->value());
  ...
  void clone(Node* node)
  {
    ...
    QSBOOST_TRY{
      qsboost::detail::allocator::construct(
...
private:
  typedef typename qsboost::detail::allocator::rebind_to<
    Allocator,Node
  >::type                                               allocator_type;
