...
#ifndef QSBOOST_MULTI_INDEX_DETAIL_INDEX_BASE_HPP
#define QSBOOST_MULTI_INDEX_DETAIL_INDEX_BASE_HPP
...
#include <qsboost/multi_index/detail/copy_map.hpp>
#include <qsboost/multi_index/detail/do_not_copy_elements_tag.hpp>
#include <qsboost/multi_index/detail/node_type.hpp>
...
#include <qsboost/multi_index/detail/index_loader.hpp>
#include <qsboost/multi_index/detail/index_saver.hpp>
#endif
...
protected:
  ...
  final_node_type* insert_(const value_type& v,final_node_type*& x,lvalue_tag)
  {
    ...
    QSBOOST_TRY{
      qsboost::detail::allocator::construct(&x->value(),v);
    }
    QSBOOST_CATCH(...){
  ...
  final_node_type* insert_(const value_type& v,final_node_type*& x,rvalue_tag)
  {
    ...
    QSBOOST_TRY{
      ...
      new (&x->value()) value_type(qsboost::move(const_cast<value_type&>(v)));
    }
    QSBOOST_CATCH(...){
      final().deallocate_node(x);
      QSBOOST_RETHROW;
  ...
  {
    x->value()=qsboost::move(const_cast<value_type&>(v));
    return true;
  }
  ...
  {
    return final().emplace_(QSBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
  }
  ...
  std::pair<final_node_type*,bool> final_emplace_hint_(
    final_node_type* position,QSBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    return final().emplace_hint_(
      position,QSBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
  }
