...
#ifndef QSBOOST_MULTI_INDEX_DETAIL_SAFE_MODE_HPP
#define QSBOOST_MULTI_INDEX_DETAIL_SAFE_MODE_HPP
...
#undef QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT
#define QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(expr,error_code) ((void)0)
#else
#if !defined(QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT)
#include <qsboost/assert.hpp>
#define QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(expr,error_code) QSBOOST_ASSERT(expr)
#endif
#endif

#define QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(it)                           \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_valid_iterator(it),                                     \
    safe_mode::invalid_iterator);

#define QSBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(it)                 \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_dereferenceable_iterator(it),                           \
    safe_mode::not_dereferenceable_iterator);

#define QSBOOST_MULTI_INDEX_CHECK_INCREMENTABLE_ITERATOR(it)                   \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_incrementable_iterator(it),                             \
    safe_mode::not_incrementable_iterator);

#define QSBOOST_MULTI_INDEX_CHECK_DECREMENTABLE_ITERATOR(it)                   \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_decrementable_iterator(it),                             \
    safe_mode::not_decrementable_iterator);

#define QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(it,cont)                            \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_is_owner(it,cont),                                      \
    safe_mode::not_owner);

#define QSBOOST_MULTI_INDEX_CHECK_SAME_OWNER(it0,it1)                          \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_same_owner(it0,it1),                                    \
    safe_mode::not_same_owner);

#define QSBOOST_MULTI_INDEX_CHECK_VALID_RANGE(it0,it1)                         \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_valid_range(it0,it1),                                   \
    safe_mode::invalid_range);

#define QSBOOST_MULTI_INDEX_CHECK_OUTSIDE_RANGE(it,it0,it1)                    \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_outside_range(it,it0,it1),                              \
    safe_mode::inside_range);

#define QSBOOST_MULTI_INDEX_CHECK_IN_BOUNDS(it,n)                              \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
    safe_mode::check_in_bounds(it,n),                                        \
    safe_mode::out_of_bounds);

#define QSBOOST_MULTI_INDEX_CHECK_DIFFERENT_CONTAINER(cont0,cont1)             \
  QSBOOST_MULTI_INDEX_SAFE_MODE_ASSERT(                                        \
...
#include <qsboost/multi_index/detail/iter_adaptor.hpp>
#include <qsboost/multi_index/safe_mode_errors.hpp>
...
#if defined(QSBOOST_HAS_THREADS)
#include <qsboost/detail/lightweight_mutex.hpp>
#endif
...
inline void detach_equivalent_iterators(Iterator& it)
{
  if(it.valid()){
    {
#if defined(QSBOOST_HAS_THREADS)
      qsboost::detail::lightweight_mutex::scoped_lock lock(it.cont->mutex);
#endif
...
QSBOOST_MULTI_INDEX_PRIVATE_IF_MEMBER_TEMPLATE_FRIENDS:
...
#if defined(QSBOOST_HAS_THREADS)
  qsboost::detail::lightweight_mutex mutex;
#endif
};
...
void safe_iterator_base::attach(safe_container_base* cont_)
{
  ...
  if(cont){
#if defined(QSBOOST_HAS_THREADS)
    qsboost::detail::lightweight_mutex::scoped_lock lock(cont->mutex);
#endif
...
public:
  ...
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(x);
...
private:
  friend class qsboost::multi_index::detail::iter_adaptor_access;

  reference dereference()const
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    QSBOOST_MULTI_INDEX_CHECK_DEREFERENCEABLE_ITERATOR(*this);
  ...
  bool equal(const safe_iterator& x)const
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(x);
    QSBOOST_MULTI_INDEX_CHECK_SAME_OWNER(*this,x);
  ...
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    QSBOOST_MULTI_INDEX_CHECK_INCREMENTABLE_ITERATOR(*this);
    ++(this->base_reference());
  }
  ...
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    QSBOOST_MULTI_INDEX_CHECK_DECREMENTABLE_ITERATOR(*this);
    --(this->base_reference());
  }
  ...
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(*this);
    QSBOOST_MULTI_INDEX_CHECK_IN_BOUNDS(*this,n);
...
template<typename Iterator,typename Container>
struct version<
  qsboost::multi_index::safe_mode::safe_iterator<Iterator,Container>
>
{
  QSBOOST_STATIC_CONSTANT(
    int,value=qsboost::serialization::version<Iterator>::value);
};
} /* namespace serialization */
#endif
