...
#ifndef QSBOOST_MULTI_INDEX_DETAIL_ORD_INDEX_IMPL_HPP
#define QSBOOST_MULTI_INDEX_DETAIL_ORD_INDEX_IMPL_HPP
...
#include <qsboost/multi_index/detail/modify_key_adaptor.hpp>
#include <qsboost/multi_index/detail/ord_index_node.hpp>
#include <qsboost/multi_index/detail/ord_index_ops.hpp>
#include <qsboost/multi_index/detail/safe_mode.hpp>
#include <qsboost/multi_index/detail/scope_guard.hpp>
#include <qsboost/multi_index/detail/unbounded.hpp>
#include <qsboost/multi_index/detail/value_compare.hpp>
#include <qsboost/multi_index/detail/vartempl_support.hpp>
#include <qsboost/multi_index/detail/ord_index_impl_fwd.hpp>
...
#include <qsboost/archive/archive_exception.hpp>
#include <qsboost/bind.hpp>
#include <qsboost/multi_index/detail/duplicates_iterator.hpp>
#include <qsboost/throw_exception.hpp> 
#endif

#if defined(QSBOOST_MULTI_INDEX_ENABLE_INVARIANT_CHECKING)
#define QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT_OF(x)                    \
...
#define QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT                          \
  QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT_OF(*this)
#else
#define QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT_OF(x)
#define QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT
#endif
...
public:
  ...
  iterator
    begin()QSBOOST_NOEXCEPT{return make_iterator(leftmost());}
  const_iterator
    begin()const QSBOOST_NOEXCEPT{return make_iterator(leftmost());}
  iterator
  ...
  {
    QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT;
    std::pair<final_node_type*,bool> p=this->final_insert_(x);
    return std::pair<iterator,bool>(make_iterator(p.first),p.second);
  }

  std::pair<iterator,bool> insert(QSBOOST_RV_REF(value_type) x)
  {
    QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT;
  ...
  iterator insert(iterator position,QSBOOST_RV_REF(value_type) x)
  {
    QSBOOST_MULTI_INDEX_CHECK_VALID_ITERATOR(position);
    QSBOOST_MULTI_INDEX_CHECK_IS_OWNER(position,*this);
    QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT;
  ...
  {
    QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT;
    QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT_OF(x);
  ...
  range(LowerBounder lower,UpperBounder upper)const
  {
    typedef typename mpl::if_<
      ...
      QSBOOST_DEDUCED_TYPENAME mpl::if_<
...
private:
  ...
  std::pair<iterator,bool> emplace_impl(QSBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT;
    ...
    iterator position,QSBOOST_MULTI_INDEX_FUNCTION_PARAM_PACK)
  {
    ...
    std::pair<final_node_type*,bool>p=
      this->final_emplace_hint_(
        ...
        QSBOOST_MULTI_INDEX_FORWARD_PARAM_PACK);
    return make_iterator(p.first);
  }
...
public:
  ...
  ordered_index& operator=(
    std::initializer_list<QSBOOST_DEDUCED_TYPENAME super::value_type> list)
  {
    this->final()=list;
    return *this;
  }
...
inline qsboost::mpl::true_* qsboost_foreach_is_noncopyable(
  qsboost::multi_index::detail::ordered_index<
    KeyFromValue,Compare,SuperMeta,TagList,Category,AugmentPolicy>*&,
...
#undef QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT
#undef QSBOOST_MULTI_INDEX_ORD_INDEX_CHECK_INVARIANT_OF
