...
#ifndef QSBOOST_IOSTREAMS_CONCEPTS_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_CONCEPTS_HPP_INCLUDED
...
#include <qsboost/iostreams/detail/default_arg.hpp>
...
template<typename Mode, typename Ch = QSBOOST_IOSTREAMS_DEFAULT_ARG(char)>
struct device {
    ...
    void close()
    {
        ...
        QSBOOST_STATIC_ASSERT((!is_convertible<Mode, two_sequence>::value));
    }

    void close(QSBOOST_IOS::openmode)
    {
        using namespace detail;
        QSBOOST_STATIC_ASSERT((is_convertible<Mode, two_sequence>::value));
    }
...
template<typename Mode, typename Ch = QSBOOST_IOSTREAMS_DEFAULT_ARG(wchar_t)>
...
struct filter {
    ...
    void close(Device&)
    {
        using namespace detail;
        QSBOOST_STATIC_ASSERT((!is_convertible<Mode, two_sequence>::value));
        QSBOOST_STATIC_ASSERT((!is_convertible<Mode, dual_use>::value));
    }

    template<typename Device>
    void close(Device&, QSBOOST_IOS::openmode)
    {
        using namespace detail;
