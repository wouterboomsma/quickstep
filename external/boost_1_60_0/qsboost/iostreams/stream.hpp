...
#ifndef QSBOOST_IOSTREAMS_STREAM_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_STREAM_HPP_INCLUDED
...
#include <qsboost/iostreams/detail/iostream.hpp>  // standard streams.
#include <qsboost/iostreams/detail/select.hpp>
#include <qsboost/iostreams/stream_buffer.hpp>
...
struct stream_traits {
    ...
    typedef typename
            iostreams::select< // Disambiguation required for Tru64.
                ...
                >,
                QSBOOST_IOSTREAMS_BASIC_IOSTREAM(char_type, traits_type),
                is_convertible<mode, input>,
                QSBOOST_IOSTREAMS_BASIC_ISTREAM(char_type, traits_type),
                else_,
                QSBOOST_IOSTREAMS_BASIC_OSTREAM(char_type, traits_type)
            >::type stream_type;
    typedef typename
            iostreams::select< // Disambiguation required for Tru64.
                mpl::and_<
...
template< typename Device,
          typename Tr =
              QSBOOST_IOSTREAMS_CHAR_TRAITS(
                  QSBOOST_DEDUCED_TYPENAME char_type_of<Device>::type
              ),
          typename Alloc =
              std::allocator<
                  QSBOOST_DEDUCED_TYPENAME char_type_of<Device>::type
...
# include <qsboost/iostreams/detail/broken_overload_resolution/stream.hpp>
#else
...
public:
    stream() { }
    QSBOOST_IOSTREAMS_FORWARD( stream, open_impl, Device,
...
private:
    void open_impl(const Device& dev QSBOOST_IOSTREAMS_PUSH_PARAMS()) // For forwarding.
    { 
        this->clear(); 
        this->member.open(dev QSBOOST_IOSTREAMS_PUSH_ARGS()); 
    }
};
