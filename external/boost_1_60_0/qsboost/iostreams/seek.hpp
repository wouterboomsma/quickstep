...
#ifndef QSBOOST_IOSTREAMS_SEEK_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_SEEK_HPP_INCLUDED
...
#include <qsboost/iostreams/detail/ios.hpp>       // streamsize, seekdir, openmode.
...
inline std::streampos
seek( T& t, Device& dev, stream_offset off, QSBOOST_IOS::seekdir way,
      QSBOOST_IOS::openmode which = QSBOOST_IOS::in | QSBOOST_IOS::out )
{ 
    using namespace detail;
...
struct seek_impl_basic_ios {
    ...
    static std::streampos seek( T& t, stream_offset off,
                                QSBOOST_IOS::seekdir way,
                                QSBOOST_IOS::openmode which )
    {
        if ( way == QSBOOST_IOS::beg &&
                                ...
                                QSBOOST_IOS::openmode which )
    {
        if ( way == QSBOOST_IOS::beg &&
             ( off < integer_traits<std::streamoff>::const_min ||
               off > integer_traits<std::streamoff>::const_max ) )
        {
            return t.QSBOOST_IOSTREAMS_PUBSEEKPOS(offset_to_position(off));
        } else {
            return t.QSBOOST_IOSTREAMS_PUBSEEKOFF(off, way, which);
        }
    }
};
