...
#ifndef QSBOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_DETAIL_INDIRECT_STREAMBUF_HPP_INCLUDED

#include <algorithm>                             // min, max.
#include <cassert>
#include <exception>
#include <typeinfo>
#include <qsboost/config.hpp>                      // Member template friends.
#include <qsboost/detail/workaround.hpp>
#include <qsboost/iostreams/constants.hpp>
#include <qsboost/iostreams/detail/adapter/concept_adapter.hpp>
#include <qsboost/iostreams/detail/buffer.hpp>
#include <qsboost/iostreams/detail/config/wide_streams.hpp>
#include <qsboost/iostreams/detail/double_object.hpp> 
...
#include <qsboost/iostreams/detail/push.hpp>
...
// Must come last.
#include <qsboost/iostreams/detail/config/disable_warnings.hpp>  // MSVC, BCC 5.x
...
public:
    ...
    void open(const T& t QSBOOST_IOSTREAMS_PUSH_PARAMS());
    bool is_open() const;
    void close();
...
#ifndef QSBOOST_IOSTREAMS_NO_LOCALE
...
void indirect_streambuf<T, Tr, Alloc, Mode>::close()
{
    ...
    base_type* self = this;
    detail::execute_all(
        detail::call_member_close(*self, QSBOOST_IOS::in),
...
{
    if ( gptr() != 0 && way == QSBOOST_IOS::cur && which == QSBOOST_IOS::in && 
         eback() - gptr() <= off && off <= egptr() - gptr() ) 
    {   // Small seek optimization
        gbump(off);
        return obj().seek(0, QSBOOST_IOS::cur, QSBOOST_IOS::in, next_) -
               static_cast<off_type>(egptr() - gptr());
    }
    if (pptr() != 0) 
        this->QSBOOST_IOSTREAMS_PUBSYNC(); // sync() confuses VisualAge 6.
    if (way == QSBOOST_IOS::cur && gptr())
...
inline void indirect_streambuf<T, Tr, Alloc, Mode>::close_impl
    (QSBOOST_IOS::openmode which)
{
    if (which == QSBOOST_IOS::in && is_convertible<Mode, input>::value) {
        setg(0, 0, 0);
    }
    if (which == QSBOOST_IOS::out && is_convertible<Mode, output>::value) {
        sync();
        setp(0, 0);
    }
    if ( !is_convertible<category, dual_use>::value ||
         is_convertible<Mode, input>::value == (which == QSBOOST_IOS::in) )
    {
...
#include <qsboost/iostreams/detail/config/enable_warnings.hpp> // MSVC, BCC 5.x
