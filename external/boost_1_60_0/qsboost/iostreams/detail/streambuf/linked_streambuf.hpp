...
#ifndef QSBOOST_IOSTREAMS_DETAIL_LINKED_STREAMBUF_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_DETAIL_LINKED_STREAMBUF_HPP_INCLUDED
...
#include <qsboost/iostreams/detail/ios.hpp>          // openmode.
...
#define QSBOOST_IOSTREAMS_USING_PROTECTED_STREAMBUF_MEMBERS(base) \
...
template<typename Ch, typename Tr = QSBOOST_IOSTREAMS_CHAR_TRAITS(Ch) >
class linked_streambuf : public QSBOOST_IOSTREAMS_BASIC_STREAMBUF(Ch, Tr) {
protected:
...
protected:
...
#else
    public:
        typedef QSBOOST_IOSTREAMS_BASIC_STREAMBUF(Ch, Tr) base;
        QSBOOST_IOSTREAMS_USING_PROTECTED_STREAMBUF_MEMBERS(base)
#endif
    void close(QSBOOST_IOS::openmode which)
    {
        if ( which == QSBOOST_IOS::in && 
    ...
    virtual void close_impl(QSBOOST_IOS::openmode) = 0;
