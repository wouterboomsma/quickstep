...
#ifndef QSBOOST_IOSTREAMS_DETAIL_RANGE_ADAPTER_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_DETAIL_RANGE_ADAPTER_HPP_INCLUDED
...
#include <qsboost/detail/iterator.hpp>             // boost::iterator_traits.
...
private:
    ...
    typedef qsboost::detail::iterator_traits<iterator>  iter_traits;
...
std::streampos range_adapter<Mode, Range>::seek
    (stream_offset off, QSBOOST_IOS::seekdir way)
{ 
...
struct range_adapter_impl<std::forward_iterator_tag> {
        ...
        (Iter& cur, Iter& last, const Ch* s, std::streamsize n)
    {
        ...
        if (cur == last && n > 0)
            qsboost::throw_exception(write_area_exhausted());
        return n;
    }
};
