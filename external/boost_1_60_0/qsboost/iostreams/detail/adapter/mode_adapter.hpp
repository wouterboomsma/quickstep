...
#ifndef QSBOOST_IOSTREAMS_DETAIL_MODE_ADAPTER_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_DETAIL_MODE_ADAPTER_HPP_INCLUDED
...
#include <qsboost/iostreams/detail/ios.hpp>  // openmode, seekdir, int types. 
...
public:
    ...
    std::streampos seek( stream_offset off, QSBOOST_IOS::seekdir way,
                         QSBOOST_IOS::openmode which = 
                             QSBOOST_IOS::in | QSBOOST_IOS::out );
#if !QSBOOST_WORKAROUND(QSBOOST_MSVC, < 1300)
    void close();
    ...
    std::streampos seek(Device& dev, stream_offset off, QSBOOST_IOS::seekdir way)
    ...
    template<typename Device>
    void close(Device& dev, QSBOOST_IOS::openmode which)
...
{ return qsboost::iostreams::seek(t_, off, way, which); }
    ...
    template<typename Mode, typename T>
    void mode_adapter<Mode, T>::close(QSBOOST_IOS::openmode which) 
