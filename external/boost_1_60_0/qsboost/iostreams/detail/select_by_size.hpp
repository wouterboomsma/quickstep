...
#ifndef QSBOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_DETAIL_SELECT_BY_SIZE_HPP_INCLUDED
...
#define SELECT_BY_SIZE_SPEC(n) \
    namespace qsboost { namespace iostreams { namespace detail { \
      static const int QSBOOST_PP_CAT(sizeof_case_, n) = sizeof(case_<n - 1>); \
      template<> \
      struct select_by_size< QSBOOST_PP_CAT(sizeof_case_, n) > { \
          struct type { QSBOOST_STATIC_CONSTANT(int, value = n - 1); }; \
...
#define QSBOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define QSBOOST_PP_LOCAL_LIMITS (0, 20)
#include QSBOOST_PP_LOCAL_ITERATE()
#undef QSBOOST_PP_LOCAL_MACRO

//--------------Definition of SELECT_BY_SIZE----------------------------------//

#define QSBOOST_SELECT_BY_SIZE(type_, name, expr) \
    QSBOOST_STATIC_CONSTANT( \
        unsigned, \
        QSBOOST_PP_CAT(qsboost_select_by_size_temp_, name) = sizeof(expr) \
    ); \
    QSBOOST_STATIC_CONSTANT( \
        type_, \
        name = \
            ( ::qsboost::iostreams::detail::select_by_size< \
                QSBOOST_PP_CAT(qsboost_select_by_size_temp_, name) \
              >::value ) \
    ) \
    /**/
...
#if defined(QSBOOST_SELECT_BY_SIZE_MAX_CASE) && \
    QSBOOST_SELECT_BY_SIZE_MAX_CASE > SELECT_BY_SIZE_MAX_SPECIALIZED

#define QSBOOST_PP_LOCAL_MACRO(n) SELECT_BY_SIZE_SPEC(n)
#define QSBOOST_PP_LOCAL_LIMITS \
    (SELECT_BY_SIZE_MAX_SPECIALIZED, QSBOOST_SELECT_BY_SIZE_MAX_CASE) \
    /**/
#include QSBOOST_PP_LOCAL_ITERATE()
#undef QSBOOST_PP_LOCAL_MACRO
#undef SELECT_BY_SIZE_MAX_SPECIALIZED
#define SELECT_BY_SIZE_MAX_SPECIALIZED QSBOOST_SELECT_BY_SIZE_MAX_CASE
