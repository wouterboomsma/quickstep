...
#ifndef QSBOOST_IOSTREAMS_DETAIL_WRAP_UNWRAP_HPP_INCLUDED
#define QSBOOST_IOSTREAMS_DETAIL_WRAP_UNWRAP_HPP_INCLUDED

#if defined(_MSC_VER) && (_MSC_VER >= 1020)
# pragma once
#endif              

#include <qsboost/config.hpp>                             // SFINAE, MSVC.
...
//------------------Definition of wrap----------------------------------------//
    ...
    template<typename T>
    inline T wrap(const T& t QSBOOST_IOSTREAMS_DISABLE_IF_STREAM(T)) 
    { return t; }

    template<typename T>
    inline typename wrapped_type<T>::type
    wrap(T& t QSBOOST_IOSTREAMS_ENABLE_IF_STREAM(T)) { return qsboost::ref(t); }
#else // #ifndef BOOST_NO_SFINAE //-------------------------------------------//
    template<typename T>
    inline typename wrapped_type<T>::type // BCC 5.x needs namespace qualification.
    wrap_impl(const T& t, mpl::true_) { return qsboost::ref(const_cast<T&>(t)); }

    template<typename T>
    inline typename wrapped_type<T>::type // BCC 5.x needs namespace qualification.
    wrap_impl(T& t, mpl::true_) { return qsboost::ref(t); }
