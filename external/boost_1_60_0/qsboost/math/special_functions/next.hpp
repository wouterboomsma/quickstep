...
#ifndef QSBOOST_MATH_SPECIAL_NEXT_HPP
#define QSBOOST_MATH_SPECIAL_NEXT_HPP
...
#include "xmmintrin.h"
#define QSBOOST_MATH_CHECK_SSE2
#endif
#endif
...
T float_next_imp(const T& val, const Policy& pol)
{
   ...
   int fpclass = (qsboost::math::fpclassify)(val);
...
inline double float_next(const double& val, const Policy& pol)
{
   ...
   if(!(qsboost::math::isfinite)(val) && (val > 0))
...
inline typename tools::promote_args<T, U>::type nextafter(const T& val, const U& direction, const Policy& pol)
{
   ...
   return val < direction ? qsboost::math::float_next<result_type>(val, pol) : val == direction ? val : qsboost::math::float_prior<result_type>(val, pol);
}
...
T float_distance_imp(const T& a, const T& b, const Policy& pol)
{
   ...
   if(!(qsboost::math::isfinite)(a))
      return policies::raise_domain_error<T>(
         function,
         "Argument a must be finite, but got %1%", a, pol);
   if(!(qsboost::math::isfinite)(b))
      return policies::raise_domain_error<T>(
         function,
   ...
   if(qsboost::math::sign(a) != qsboost::math::sign(b))
   ...
   //
   frexp(((qsboost::math::fpclassify)(a) == (int)FP_SUBNORMAL) ? tools::min_value<T>() : a, &expon);
   ...
   //
   T mb, x, y, z;
   if(((qsboost::math::fpclassify)(a) == (int)FP_SUBNORMAL) || (b - a < tools::min_value<T>()))
   {
      //
   ...
   //
   QSBOOST_ASSERT(result == floor(result));
   return result;
}
...
{
   return qsboost::math::float_distance(a, b, policies::policy<>());
}
...
{
   return qsboost::math::float_advance(val, distance, policies::policy<>());
}
