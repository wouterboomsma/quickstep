...
#ifndef QSBOOST_MATH_TOOLS_SIGN_HPP
#define QSBOOST_MATH_TOOLS_SIGN_HPP
...
namespace detail {
  ...
  // signbit
    ...
    {
       return (qsboost::math::signbit)(static_cast<double>(x));
    }
    ...
    inline long double (changesign_impl)(long double x, generic_tag<true> const&)
    {
       ...
       pd[0] = qsboost::math::changesign(pd[0]);
       pd[1] = qsboost::math::changesign(pd[1]);
       return x;
    }
    ...
    {
        typedef QSBOOST_DEDUCED_TYPENAME fp_traits<T>::sign_change_type traits;
...
template <class T>
inline int sign QSBOOST_NO_MACRO_EXPAND(const T& z)
{
   return (z == 0) ? 0 : (qsboost::math::signbit)(z) ? -1 : 1;
}
...
inline typename tools::promote_args_permissive<T, U>::type 
   copysign QSBOOST_NO_MACRO_EXPAND(const T& x, const U& y)
{
   QSBOOST_MATH_STD_USING
   typedef typename tools::promote_args_permissive<T, U>::type result_type;
   return (qsboost::math::signbit)(static_cast<result_type>(x)) != (qsboost::math::signbit)(static_cast<result_type>(y)) 
      ? (qsboost::math::changesign)(static_cast<result_type>(x)) : static_cast<result_type>(x);
}
