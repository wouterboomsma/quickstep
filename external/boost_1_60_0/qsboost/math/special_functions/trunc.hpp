...
#ifndef QSBOOST_MATH_TRUNC_HPP
#define QSBOOST_MATH_TRUNC_HPP
...
namespace qsboost{ namespace math{ namespace detail{
...
inline int itrunc(const T& v, const Policy& pol)
{
   QSBOOST_MATH_STD_USING
   typedef typename tools::promote_args<T>::type result_type;
   result_type r = qsboost::math::trunc(v, pol);
...
template <class T, class Policy>
inline qsboost::long_long_type lltrunc(const T& v, const Policy& pol)
{
   QSBOOST_MATH_STD_USING
   typedef typename tools::promote_args<T>::type result_type;
   result_type r = qsboost::math::trunc(v, pol);
   if((r > (std::numeric_limits<qsboost::long_long_type>::max)()) || (r < (std::numeric_limits<qsboost::long_long_type>::min)()))
      return static_cast<qsboost::long_long_type>(policies::raise_rounding_error("boost::math::lltrunc<%1%>(%1%)", 0, v, static_cast<qsboost::long_long_type>(0), pol));
   return static_cast<qsboost::long_long_type>(r);
}
template <class T>
inline qsboost::long_long_type lltrunc(const T& v)
{
