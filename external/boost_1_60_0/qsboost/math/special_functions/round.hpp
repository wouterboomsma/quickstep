...
#ifndef QSBOOST_MATH_ROUND_HPP
#define QSBOOST_MATH_ROUND_HPP
...
template <class T, class Policy>
inline qsboost::long_long_type llround(const T& v, const Policy& pol)
{
   QSBOOST_MATH_STD_USING
   T r = qsboost::math::round(v, pol);
   if((r > (std::numeric_limits<qsboost::long_long_type>::max)()) || (r < (std::numeric_limits<qsboost::long_long_type>::min)()))
      return static_cast<qsboost::long_long_type>(policies::raise_rounding_error("boost::math::llround<%1%>(%1%)", 0, v, static_cast<qsboost::long_long_type>(0), pol));
   return static_cast<qsboost::long_long_type>(r);
}
template <class T>
inline qsboost::long_long_type llround(const T& v)
{
