...
#ifndef QSBOOST_MATH_FP_TRAITS_HPP
#define QSBOOST_MATH_FP_TRAITS_HPP
...
#   define QSBOOST_FPCLASSIFY_VAX_FORMAT
#endif
...
#include <qsboost/detail/endian.hpp>
...
#           define QSBOOST_FPCLASSIFY_PREFIX ::__std_alias:: 
#        else
#           define QSBOOST_FPCLASSIFY_PREFIX ::_STLP_VENDOR_CSTD:: 
#        endif
#     else
#        define QSBOOST_FPCLASSIFY_PREFIX ::std::
#     endif
#  else
#     undef QSBOOST_HAS_FPCLASSIFY
#     define QSBOOST_FPCLASSIFY_PREFIX
#  endif
...
#ifndef QSBOOST_FPCLASSIFY_VAX_FORMAT

template<> struct fp_traits_non_native<float, single_precision>
{
    typedef ieee_copy_all_bits_tag method;

    QSBOOST_STATIC_CONSTANT(uint32_t, sign        = 0x80000000u);
    QSBOOST_STATIC_CONSTANT(uint32_t, exponent    = 0x7f800000);
    QSBOOST_STATIC_CONSTANT(uint32_t, flag        = 0x00000000);
    QSBOOST_STATIC_CONSTANT(uint32_t, significand = 0x007fffff);
...
#if defined(QSBOOST_NO_INT64_T) || defined(QSBOOST_NO_INCLASS_MEMBER_INITIALIZATION) \
...
template<> struct fp_traits_non_native<double, double_precision>
{
    ...
    QSBOOST_STATIC_CONSTANT(uint32_t, sign        = 0x80000000u);
    QSBOOST_STATIC_CONSTANT(uint32_t, exponent    = 0x7ff00000);
    QSBOOST_STATIC_CONSTANT(uint32_t, flag        = 0);
    QSBOOST_STATIC_CONSTANT(uint32_t, significand = 0x000fffff);
...
#if defined(QSBOOST_BIG_ENDIAN)
    QSBOOST_STATIC_CONSTANT(int, offset_ = 0);
#elif defined(QSBOOST_LITTLE_ENDIAN)
    QSBOOST_STATIC_CONSTANT(int, offset_ = 4);
#else
    QSBOOST_STATIC_ASSERT(false);
#endif
};
...
struct fp_traits_non_native<long double, extended_double_precision>
{
    ...
    QSBOOST_STATIC_CONSTANT(uint32_t, sign        = 0x80000000u);
    QSBOOST_STATIC_CONSTANT(uint32_t, exponent    = 0x7fff0000);
    QSBOOST_STATIC_CONSTANT(uint32_t, flag        = 0x00008000);
    QSBOOST_STATIC_CONSTANT(uint32_t, significand = 0x00007fff);
...
private:
    ...
    QSBOOST_STATIC_CONSTANT(int, offset_ = 0);
#elif defined(QSBOOST_LITTLE_ENDIAN)
    QSBOOST_STATIC_CONSTANT(int, offset_ = 12);
#else
    QSBOOST_STATIC_ASSERT(false);
#endif
};
...
struct fp_traits_non_native<long double, extended_double_precision>
{
    ...
    QSBOOST_STATIC_CONSTANT(uint32_t, exponent    = 0x7fff0000);
    QSBOOST_STATIC_CONSTANT(uint32_t, flag        = 0x00000000);
    QSBOOST_STATIC_CONSTANT(uint32_t, significand = 0x0000ffff);
...
{
    typedef QSBOOST_DEDUCED_TYPENAME size_to_precision<sizeof(T), ::qsboost::is_floating_point<T>::value>::type precision;
...
#if (defined(QSBOOST_MATH_USE_C99) && !(defined(__GNUC__) && (__GNUC__ < 4))) \
   && !defined(__hpux) \
   && !defined(__DECCXX)\
   && !defined(__osf__) \
   && !defined(__SGI_STL_PORT) && !defined(_STLPORT_VERSION)\
   && !defined(__FAST_MATH__)\
   && !defined(QSBOOST_MATH_DISABLE_STD_FPCLASSIFY)\
   && !defined(QSBOOST_INTEL)\
   && !defined(sun)
#  define QSBOOST_MATH_USE_STD_FPCLASSIFY
#endif

template<class T> struct fp_traits
{
    typedef QSBOOST_DEDUCED_TYPENAME size_to_precision<sizeof(T), ::qsboost::is_floating_point<T>::value>::type precision;
#if defined(QSBOOST_MATH_USE_STD_FPCLASSIFY) && !defined(QSBOOST_MATH_DISABLE_STD_FPCLASSIFY)
