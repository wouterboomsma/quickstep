...
#ifndef QSBOOST_PROTO_FWD_HPP_EAN_04_01_2005
#define QSBOOST_PROTO_FWD_HPP_EAN_04_01_2005
...
#ifndef QSBOOST_PROTO_MAX_ARITY
# define QSBOOST_PROTO_MAX_ARITY 10
#endif

#ifndef QSBOOST_PROTO_MAX_LOGICAL_ARITY
# define QSBOOST_PROTO_MAX_LOGICAL_ARITY 10
#endif

#ifndef QSBOOST_PROTO_MAX_FUNCTION_CALL_ARITY
# define QSBOOST_PROTO_MAX_FUNCTION_CALL_ARITY QSBOOST_PROTO_MAX_ARITY
#endif

#if QSBOOST_PROTO_MAX_ARITY < 3
# error QSBOOST_PROTO_MAX_ARITY must be at least 3
#endif

#if QSBOOST_PROTO_MAX_FUNCTION_CALL_ARITY > QSBOOST_PROTO_MAX_ARITY
# error QSBOOST_PROTO_MAX_FUNCTION_CALL_ARITY cannot be larger than QSBOOST_PROTO_MAX_ARITY
#endif

#ifndef QSBOOST_PROTO_DONT_USE_PREPROCESSED_FILES
  #if 10 < QSBOOST_PROTO_MAX_ARITY ||                                                                 \
      10 < QSBOOST_PROTO_MAX_LOGICAL_ARITY ||                                                         \
      10 < QSBOOST_PROTO_MAX_FUNCTION_CALL_ARITY
    #define QSBOOST_PROTO_DONT_USE_PREPROCESSED_FILES
  #endif
#endif

#ifndef QSBOOST_PROTO_BROKEN_CONST_OVERLOADS
# if QSBOOST_WORKAROUND(__GNUC__, == 3) \
  || QSBOOST_WORKAROUND(__EDG_VERSION__, QSBOOST_TESTED_AT(310))
#  define QSBOOST_PROTO_BROKEN_CONST_OVERLOADS
# endif
#endif

#ifndef QSBOOST_PROTO_BROKEN_CONST_QUALIFIED_FUNCTIONS
# if QSBOOST_WORKAROUND(__GNUC__, == 3) \
  || QSBOOST_WORKAROUND(__EDG_VERSION__, QSBOOST_TESTED_AT(310))
#  define QSBOOST_PROTO_BROKEN_CONST_QUALIFIED_FUNCTIONS
# endif
#endif

#ifdef QSBOOST_PROTO_BROKEN_CONST_OVERLOADS
# include <qsboost/utility/enable_if.hpp>
# include <qsboost/type_traits/is_const.hpp>
# define QSBOOST_PROTO_DISABLE_IF_IS_CONST(T)\
    , typename qsboost::disable_if_c<qsboost::is_const<T>::value, qsboost::proto::detail::undefined>::type * = 0
#else
# define QSBOOST_PROTO_DISABLE_IF_IS_CONST(T)
#endif

#ifdef QSBOOST_PROTO_BROKEN_CONST_QUALIFIED_FUNCTIONS
# include <qsboost/utility/enable_if.hpp>
# include <qsboost/type_traits/is_function.hpp>
# define QSBOOST_PROTO_DISABLE_IF_IS_FUNCTION(T)\
    , typename qsboost::disable_if_c<qsboost::is_function<T>::value, qsboost::proto::detail::undefined>::type * = 0
#else
# define QSBOOST_PROTO_DISABLE_IF_IS_FUNCTION(T)
#endif

#ifndef QSBOOST_PROTO_BROKEN_PTS
# if QSBOOST_WORKAROUND(QSBOOST_MSVC, <= 1400)
#  define QSBOOST_PROTO_BROKEN_PTS
# endif
#endif

#ifdef QSBOOST_NO_CXX11_DECLTYPE_N3276
...
# ifndef QSBOOST_PROTO_USE_NORMAL_RESULT_OF
#  define QSBOOST_PROTO_USE_NORMAL_RESULT_OF
# endif
#endif

// Unless compiler support is there, use tr1_result_of instead of
// result_of to avoid the problems addressed by N3276.
#ifdef QSBOOST_PROTO_USE_NORMAL_RESULT_OF
# define QSBOOST_PROTO_RESULT_OF qsboost::result_of
#else
# define QSBOOST_PROTO_RESULT_OF qsboost::tr1_result_of
#endif

// If we're using the decltype-based result_of, we need to be a bit
// stricter about the return types of some functions.
#if defined(QSBOOST_RESULT_OF_USE_DECLTYPE) && defined(QSBOOST_PROTO_USE_NORMAL_RESULT_OF)
# define QSBOOST_PROTO_STRICT_RESULT_OF
# define QSBOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(X, Y) X
#else
# define QSBOOST_PROTO_RETURN_TYPE_STRICT_LOOSE(X, Y) Y
#endif

#ifdef QSBOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING
# define QSBOOST_PROTO_EXTENDED_TEMPLATE_PARAMETERS_MATCHING 
#endif

#if defined(_MSC_VER)
# define QSBOOST_PROTO_DISABLE_MSVC_C4522 __pragma(warning(disable : 4522))  // 'class' : multiple assignment operators specified
# define QSBOOST_PROTO_DISABLE_MSVC_C4714 __pragma(warning(disable : 4714))  // function 'xxx' marked as __forceinline not inlined
#else
# define QSBOOST_PROTO_DISABLE_MSVC_C4522 
# define QSBOOST_PROTO_DISABLE_MSVC_C4714
#endif
...
namespace qsboost { namespace proto
{
    namespace detail
    {
        ...
        /// INTERNAL ONLY
        ///
        #define QSBOOST_PROTO_UNCVREF(X)                                                              \
            typename qsboost::proto::detail::uncvref<X>::type                                         \
            /**/
    ...
    namespace argsns_
    {
        ...
        #define M0(Z, N, DATA)                                                                      \
        template<QSBOOST_PP_ENUM_PARAMS_Z(Z, N, typename Arg)> struct QSBOOST_PP_CAT(list, N);          \
        /**/
        QSBOOST_PP_REPEAT_FROM_TO(1, QSBOOST_PP_INC(QSBOOST_PROTO_MAX_ARITY), M0, ~)
        #undef M0
    }
    ...
    template<QSBOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(QSBOOST_PROTO_MAX_LOGICAL_ARITY, typename G, void)>
    struct or_;
    ...
    template<typename Tag, QSBOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(QSBOOST_PROTO_MAX_ARITY, typename A, void)>
    struct nary_expr;
    ...
    template<QSBOOST_PP_ENUM_PARAMS_WITH_A_DEFAULT(QSBOOST_PROTO_MAX_ARITY, typename A, void)>
    struct function;
    ...
    #define QSBOOST_PROTO_UNEXPR() typedef int proto_is_expr_;
    #define QSBOOST_PROTO_CALLABLE() typedef void proto_is_callable_;
    #define QSBOOST_PROTO_AGGREGATE() typedef void proto_is_aggregate_;
    #define QSBOOST_PROTO_USE_BASIC_EXPR() typedef void proto_use_basic_expr_;
    ...
    #define M0(Z, N, DATA) typedef _child_c<N> QSBOOST_PP_CAT(_child, N);
    QSBOOST_PP_REPEAT_FROM_TO(
        2
      , QSBOOST_PP_DEC(QSBOOST_PROTO_MAX_ARITY)
      , M0
      , ~
    )
    #undef M0
