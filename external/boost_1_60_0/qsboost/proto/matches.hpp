...
#ifndef QSBOOST_PROTO_MATCHES_HPP_EAN_11_03_2006
#define QSBOOST_PROTO_MATCHES_HPP_EAN_11_03_2006
...
#define QSBOOST_PROTO_LOGICAL_typename_G  QSBOOST_PP_ENUM_PARAMS(QSBOOST_PROTO_MAX_LOGICAL_ARITY, typename G)
#define QSBOOST_PROTO_LOGICAL_G           QSBOOST_PP_ENUM_PARAMS(QSBOOST_PROTO_MAX_LOGICAL_ARITY, G)
...
namespace qsboost { namespace proto
{
    ...
    namespace detail
    {
        ...
        template<typename T, typename U
            QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(long Arity = detail::template_arity<U>::value)
        >
        ...
        template<typename T>
        struct lambda_matches<T, proto::_ QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<typename T>
        struct lambda_matches<T, T QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<typename T, std::size_t M, typename U>
        struct lambda_matches<T[M], U QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : array_matches<T[M], U>
        {};

        template<typename T, std::size_t M>
        struct lambda_matches<T[M], _ QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<typename T, std::size_t M>
        struct lambda_matches<T[M], T[M] QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(-1)>
          : mpl::true_
        {};

        template<template<typename> class T, typename Expr0, typename Grammar0>
        struct lambda_matches<T<Expr0>, T<Grammar0> QSBOOST_PROTO_TEMPLATE_ARITY_PARAM(1) >
        ...
        template<
            typename T
          , typename U
          , bool B = is_array<QSBOOST_PROTO_UNCVREF(T)>::value
        >
        ...
        struct terminal_matches
          : mpl::or_<
                mpl::and_<
                  ...
                  , lambda_matches<
                        QSBOOST_PROTO_UNCVREF(T)
                      , QSBOOST_PROTO_UNCVREF(U)
                    >
                >
        ...
        #include <qsboost/proto/detail/and_n.hpp>
        #include <qsboost/proto/detail/or_n.hpp>
        #include <qsboost/proto/detail/matches_.hpp>
        #include <qsboost/proto/detail/vararg_matches_impl.hpp>
        #include <qsboost/proto/detail/lambda_matches.hpp>
    ...
    template<QSBOOST_PROTO_LOGICAL_typename_G>
    struct or_ : transform<or_<QSBOOST_PROTO_LOGICAL_G> >
    {
    ...
    template<QSBOOST_PROTO_LOGICAL_typename_G>
    struct and_ : transform<and_<QSBOOST_PROTO_LOGICAL_G> >
    {
    ...
    ///
    template<QSBOOST_PROTO_LOGICAL_typename_G>
    struct is_callable<or_<QSBOOST_PROTO_LOGICAL_G> >
      : mpl::true_
    {};
    ...
    ///
    template<QSBOOST_PROTO_LOGICAL_typename_G>
    struct is_callable<and_<QSBOOST_PROTO_LOGICAL_G> >
      : mpl::true_
    {};
...
#undef QSBOOST_PROTO_LOGICAL_typename_G
#undef QSBOOST_PROTO_LOGICAL_G
