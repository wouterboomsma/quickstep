    ...
    #include <qsboost/proto/transform/detail/preprocessed/when.hpp>
    ...
    #define QSBOOST_PP_ITERATION_PARAMS_1                                                             \
        (3, (0, QSBOOST_PROTO_MAX_ARITY, <qsboost/proto/transform/detail/when.hpp>))
    ...
    /// ObjectTransforms.
    template<typename Grammar, typename R QSBOOST_PP_ENUM_TRAILING_PARAMS(N, typename A)>
    struct when<Grammar, R(QSBOOST_PP_ENUM_PARAMS(N, A))>
      : detail::when_impl<Grammar, R, R(QSBOOST_PP_ENUM_PARAMS(N, A))>
    {};
