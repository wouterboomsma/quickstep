        ...
        #include <qsboost/proto/detail/preprocessed/expr_variadic.hpp>
    #else
        #include <qsboost/proto/detail/preprocessed/expr.hpp>
    #endif
    ...
    #define QSBOOST_PROTO_CHILD(Z, N, DATA)                                                       \
        typedef QSBOOST_PP_CAT(Arg, N) QSBOOST_PP_CAT(proto_child, N);                              \
        QSBOOST_PP_CAT(proto_child, N) QSBOOST_PP_CAT(child, N);                                    \
        /**< INTERNAL ONLY */

    /// INTERNAL ONLY
    ///
    #define QSBOOST_PROTO_VOID(Z, N, DATA)                                                        \
        typedef void QSBOOST_PP_CAT(proto_child, N);                                              \
        /**< INTERNAL ONLY */
    ...
    #define QSBOOST_PROTO_DEFINE_TERMINAL
    #define QSBOOST_PP_ITERATION_PARAMS_1                                                         \
        (3, (0, 0, <qsboost/proto/detail/expr.hpp>))
    #include QSBOOST_PP_ITERATE()

    #undef QSBOOST_PROTO_DEFINE_TERMINAL
    #define QSBOOST_PP_ITERATION_PARAMS_1                                                         \
        (3, (1, QSBOOST_PROTO_MAX_ARITY, <qsboost/proto/detail/expr.hpp>))
    #include QSBOOST_PP_ITERATE()
    ...
    // Generate non-variadic versions of expr
        ...
        // The expr<> specializations are actually defined here.
        #define QSBOOST_PROTO_DEFINE_TERMINAL
        #define QSBOOST_PP_ITERATION_PARAMS_1 (3, (0, 0, <qsboost/proto/detail/expr.hpp>))
        #include QSBOOST_PP_ITERATE()

        #undef QSBOOST_PROTO_DEFINE_TERMINAL
        #define QSBOOST_PP_ITERATION_PARAMS_1 (3, (1, QSBOOST_PROTO_MAX_ARITY, <qsboost/proto/detail/expr.hpp>))
        #include QSBOOST_PP_ITERATE()
    ...
    #undef QSBOOST_PROTO_VOID

#else

    #define ARG_COUNT QSBOOST_PP_MAX(1, QSBOOST_PP_ITERATION())
    ...
    #else
    template<typename Tag QSBOOST_PP_ENUM_TRAILING_PARAMS(ARG_COUNT, typename Arg)>
    struct expr<Tag, QSBOOST_PP_CAT(list, QSBOOST_PP_ITERATION())<QSBOOST_PP_ENUM_PARAMS(ARG_COUNT, Arg)>, QSBOOST_PP_ITERATION() >
    #endif
    {
        typedef Tag proto_tag;
        static const long proto_arity_c = QSBOOST_PP_ITERATION();
        typedef mpl::long_<QSBOOST_PP_ITERATION() > proto_arity;
        typedef expr proto_base_expr;
        #ifdef QSBOOST_PROTO_DEFINE_TERMINAL
        typedef term<Arg0> proto_args;
        #else
        typedef QSBOOST_PP_CAT(list, QSBOOST_PP_ITERATION())<QSBOOST_PP_ENUM_PARAMS(ARG_COUNT, Arg)> proto_args;
        #endif
        typedef basic_expr<Tag, proto_args, QSBOOST_PP_ITERATION() > proto_grammar;
        ...
        QSBOOST_PP_REPEAT(ARG_COUNT, QSBOOST_PROTO_CHILD, ~)
        QSBOOST_PP_REPEAT_FROM_TO(ARG_COUNT, QSBOOST_PROTO_MAX_ARITY, QSBOOST_PROTO_VOID, ~)
        ...
        /// arguments.
        ///
        template<QSBOOST_PP_ENUM_PARAMS(ARG_COUNT, typename A)>
        QSBOOST_FORCEINLINE
        static expr const make(QSBOOST_PP_ENUM_BINARY_PARAMS(ARG_COUNT, A, const &a))
        {
            expr that = {QSBOOST_PP_ENUM_PARAMS(ARG_COUNT, a)};
            return that;
        }
    #endif

    #if 1 == QSBOOST_PP_ITERATION()
        ...
        {
            return qsboost::addressof(this->child0);
        }
    #else
        /// INTERNAL ONLY
        ///
    ...
    #define QSBOOST_PP_ITERATION_PARAMS_2                                                             \
        (3, (1, QSBOOST_PP_DEC(QSBOOST_PROTO_MAX_FUNCTION_CALL_ARITY), <qsboost/proto/detail/expr_funop.hpp>))
    #include QSBOOST_PP_ITERATE()
