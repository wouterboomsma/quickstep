...
#ifndef QSBOOST_MPL_AUX_FULL_LAMBDA_HPP_INCLUDED
#define QSBOOST_MPL_AUX_FULL_LAMBDA_HPP_INCLUDED
...
#   define QSBOOST_MPL_PREPROCESSED_HEADER full_lambda.hpp
...
#   define AUX778076_ARITY_PARAM(param) \
    QSBOOST_MPL_AUX_LAMBDA_ARITY_PARAM(param) \
    /**/
...
#define QSBOOST_PP_ITERATION_PARAMS_1 \
    (3,(0, QSBOOST_MPL_LIMIT_METAFUNCTION_ARITY, <qsboost/mpl/aux_/full_lambda.hpp>))
...
struct lambda<
        ...
        , Tag
        AUX778076_ARITY_PARAM(int_<QSBOOST_PP_INC(QSBOOST_MPL_LIMIT_METAFUNCTION_ARITY)>)
        >
{
    typedef false_ is_le;
...
#elif !defined(QSBOOST_MPL_CFG_DMC_AMBIGUOUS_CTPS)
...
#if !defined(QSBOOST_MPL_CFG_EXTENDED_TEMPLATE_PARAMETERS_MATCHING)
QSBOOST_MPL_AUX_NA_SPEC(2, lambda)
#else
QSBOOST_MPL_AUX_NA_SPEC2(2, 3, lambda)
#endif
...
#   define AUX778076_RESULT(unused, i_, T) \
    QSBOOST_PP_COMMA_IF(i_) \
    typename QSBOOST_PP_CAT(T, QSBOOST_PP_INC(i_))::result_ \
    /**/
...
template<
    ...
    >
struct QSBOOST_PP_CAT(le_result,i_)
{
    typedef F<
          QSBOOST_MPL_PP_REPEAT(i_, AUX778076_TYPE, L)
        > result_;
...
template<
    ...
    , AUX778076_LAMBDA_PARAMS(i_, typename L)
    >
struct QSBOOST_PP_CAT(le_result,i_)< true_,Tag,F,AUX778076_LAMBDA_PARAMS(i_, L) >
{
    typedef QSBOOST_PP_CAT(bind,i_)<
          QSBOOST_PP_CAT(quote,i_)<F,Tag>
        , QSBOOST_MPL_PP_REPEAT(i_, AUX778076_RESULT, L)
        > result_;
...
#   define AUX778076_LAMBDA_TYPEDEF(unused, i_, T) \
    typedef lambda< QSBOOST_PP_CAT(T, QSBOOST_PP_INC(i_)), Tag > \
...
#   define AUX778076_IS_LAMBDA_EXPR(unused, i_, unused2) \
    QSBOOST_PP_COMMA_IF(i_) \
    QSBOOST_PP_CAT(is_le,QSBOOST_PP_INC(i_))::value \
/**/
...
struct lambda< 
        ...
        >
{
    QSBOOST_MPL_PP_REPEAT(i_, AUX778076_LAMBDA_TYPEDEF, T)
    ...
    typedef aux::QSBOOST_PP_CAT(le_result,i_)<
...
struct lambda<
        ...
        , Tag
        AUX778076_ARITY_PARAM(int_<QSBOOST_PP_INC(i_)>)
        >
{
    typedef false_ is_le;
    typedef QSBOOST_PP_CAT(bind,i_)<
