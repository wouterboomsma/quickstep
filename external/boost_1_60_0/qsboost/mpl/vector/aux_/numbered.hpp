...
#   define AUX778076_VECTOR_TAIL(vector, i_, T) \
    QSBOOST_PP_CAT(vector,i_)< \
          QSBOOST_PP_ENUM_PARAMS(i_, T) \
        > \
    /**/

#if i_ > 0
template<
      QSBOOST_PP_ENUM_PARAMS(i_, typename T)
    >
struct QSBOOST_PP_CAT(vector,i_)
    : v_item<
          QSBOOST_PP_CAT(T,QSBOOST_PP_DEC(i_))
        , AUX778076_VECTOR_TAIL(vector,QSBOOST_PP_DEC(i_),T)
        >
{
    typedef QSBOOST_PP_CAT(vector,i_) type;
};
#endif
...
#   define AUX778076_VECTOR_ITEM(unused, i_, unused2) \
    typedef QSBOOST_PP_CAT(T,i_) QSBOOST_PP_CAT(item,i_); \
    /**/

    QSBOOST_PP_REPEAT(i_, AUX778076_VECTOR_ITEM, unused)
#   undef AUX778076_VECTOR_ITEM
    typedef void_ QSBOOST_PP_CAT(item,i_);
    typedef QSBOOST_PP_CAT(T,QSBOOST_PP_DEC(i_)) back;
...
template<>
struct push_front_impl< aux::vector_tag<QSBOOST_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef QSBOOST_PP_CAT(vector,i_)<
              T
              QSBOOST_PP_COMMA_IF(QSBOOST_PP_DEC(i_))
              QSBOOST_PP_ENUM_PARAMS(QSBOOST_PP_DEC(i_), typename Vector::item)
            > type;
    };
};
...
struct pop_front_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef QSBOOST_PP_CAT(vector,QSBOOST_PP_DEC(i_))<
              QSBOOST_PP_ENUM_SHIFTED_PARAMS(i_, typename Vector::item)
            > type;
    };
};
...
template<>
struct push_back_impl< aux::vector_tag<QSBOOST_PP_DEC(i_)> >
{
    template< typename Vector, typename T > struct apply
    {
        typedef QSBOOST_PP_CAT(vector,i_)<
...
struct v_at<V,i_>
{
    typedef typename V::QSBOOST_PP_CAT(item,i_) type;
};
...
template<> struct v_at_impl<i_>
{
    ...
    {
        typedef typename V_::QSBOOST_PP_CAT(item,i_) type;
    };
};
}
...
struct at_impl< aux::vector_tag<i_> >
{
    ...
    {
        typedef typename aux::v_at_impl<QSBOOST_MPL_AUX_VALUE_WKND(N)::value>
