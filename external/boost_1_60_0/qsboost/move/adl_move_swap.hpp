...
#ifndef QSBOOST_MOVE_ADL_MOVE_SWAP_HPP
#define QSBOOST_MOVE_ADL_MOVE_SWAP_HPP
...
//Try to avoid including <algorithm>, as it's quite big
   ...
   #if !defined(QSBOOST_GCC) || ((__GNUC__ < 4) || ((__GNUC__ == 4) && (__GNUC_MINOR__ < 3)))
...
#include <qsboost/move/utility_core.hpp> //for boost::move

#if !defined(QSBOOST_MOVE_DOXYGEN_INVOKED)

#if defined(QSBOOST_NO_CXX11_RVALUE_REFERENCES)
namespace qsboost_move_member_swap {
...
namespace qsboost_move_adl_swap{
...
template<class T>
void swap_proxy(T& x, T& y, typename qsboost::move_detail::enable_if_c<!qsboost::move_detail::has_move_emulation_enabled_impl<T>::value>::type* = 0)
{
...
void swap_proxy(T& x, T& y
               , typename qsboost::move_detail::enable_if< and_op_not_impl<qsboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                        , qsboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  T t(::qsboost::move(x)); x = ::qsboost::move(y); y = ::qsboost::move(t);  }

template<class T>
void swap_proxy(T& x, T& y
               , typename qsboost::move_detail::enable_if< and_op_impl< qsboost::move_detail::has_move_emulation_enabled_impl<T>
                                                                    , qsboost_move_member_swap::has_member_swap<T> >
                                                       >::type* = 0)
{  x.swap(y);  }
...
void swap_proxy(T (& x)[N], T (& y)[N])
{
   for (std::size_t i = 0; i < N; ++i){
      ::qsboost_move_adl_swap::swap_proxy(x[i], y[i]);
   }
}
...
{
   ::qsboost_move_adl_swap::swap_proxy(x, y);
}
