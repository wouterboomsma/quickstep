...
#ifndef QSBOOST_UNITS_BASE_UNIT_HPP
#define QSBOOST_UNITS_BASE_UNIT_HPP
...
#include <qsboost/units/detail/ordinal.hpp>
#include <qsboost/units/detail/prevent_redefinition.hpp>
...
struct check_base_unit {
    enum {
        value =
            sizeof(qsboost_units_unit_is_registered(units::base_unit_ordinal<N>())) == sizeof(detail::yes) &&
            sizeof(qsboost_units_unit_is_registered(units::base_unit_pair<T, N>())) != sizeof(detail::yes)
    };
};
...
template<class Derived,
         class Dim,
         long N
#if !defined(QSBOOST_UNITS_DOXYGEN) && !defined(__BORLANDC__)
         ,
    ...
    public ordinal<N> 
{
    ...
    private:
        ...
        /// INTERNAL ONLY
        friend detail::yes 
        qsboost_units_unit_is_registered(const units::base_unit_ordinal<N>&) 
        { detail::yes result; return(result); }
        
        /// But make sure we can identify the current instantiation!
        /// INTERNAL ONLY
        friend detail::yes 
        qsboost_units_unit_is_registered(const units::base_unit_pair<Derived, N>&) 
        { detail::yes result; return(result); }
};
