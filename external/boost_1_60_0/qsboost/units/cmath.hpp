...
#ifndef QSBOOST_UNITS_CMATH_HPP 
#define QSBOOST_UNITS_CMATH_HPP

#include <qsboost/config/no_tr1/cmath.hpp>
#include <cstdlib>

#include <qsboost/math/special_functions/fpclassify.hpp>
#include <qsboost/math/special_functions/hypot.hpp>
#include <qsboost/math/special_functions/next.hpp>
#include <qsboost/math/special_functions/round.hpp>
#include <qsboost/math/special_functions/sign.hpp>

#include <qsboost/units/dimensionless_quantity.hpp>
#include <qsboost/units/pow.hpp>
#include <qsboost/units/quantity.hpp>
#include <qsboost/units/detail/cmath_impl.hpp>
#include <qsboost/units/detail/dimensionless_unit.hpp>

#include <qsboost/units/systems/si/plane_angle.hpp>
...
inline 
bool 
isfinite QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{ 
    using qsboost::math::isfinite;
    return isfinite QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value());
}

template<class Unit,class Y>
inline 
bool 
isinf QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{ 
    using qsboost::math::isinf;
    return isinf QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value());
}

template<class Unit,class Y>
inline
bool 
isnan QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{ 
    using qsboost::math::isnan;
    return isnan QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value());
}

template<class Unit,class Y>
inline 
bool 
isnormal QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{ 
    using qsboost::math::isnormal;
    return isnormal QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value());
}

template<class Unit,class Y>
inline 
bool 
isgreater QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                            const quantity<Unit,Y>& q2)
{ 
    using namespace detail;
    return isgreater QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value());
}

template<class Unit,class Y>
inline 
bool 
isgreaterequal QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                                 const quantity<Unit,Y>& q2)
{ 
    using namespace detail;
    return isgreaterequal QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value());
}

template<class Unit,class Y>
inline 
bool 
isless QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                         const quantity<Unit,Y>& q2)
{ 
    using namespace detail;
    return isless QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value());
}

template<class Unit,class Y>
inline 
bool 
islessequal QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                              const quantity<Unit,Y>& q2)
{ 
    using namespace detail;
    return islessequal QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value());
}

template<class Unit,class Y>
inline 
bool 
islessgreater QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                                const quantity<Unit,Y>& q2)
{ 
    using namespace detail;
    return islessgreater QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value());
}

template<class Unit,class Y>
inline 
bool 
isunordered QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                              const quantity<Unit,Y>& q2)
{ 
    using namespace detail;
    return isunordered QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value());
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
abs QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using std::abs;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(abs QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
ceil QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using std::ceil;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(ceil QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
copysign QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
         const quantity<Unit,Y>& q2)
{
    ...
    return quantity_type::from_value(copysign QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value()));
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
fabs QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using std::fabs;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(fabs QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
floor QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using std::floor;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(floor QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
fdim QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                       const quantity<Unit,Y>& q2)
{
    using namespace detail;
    ...
    return quantity_type::from_value(fdim QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value()));
}
...
fma QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit1,Y>& q1,
                                      ...
                                      const quantity<Unit3,Y>& q3)
{
    ...
    return quantity_type::from_value(fma QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value(),q3.value()));
}
...
inline 
quantity<Unit,Y> 
fmax QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                       const quantity<Unit,Y>& q2)
{
    using namespace detail;
    ...
    return quantity_type::from_value(fmax QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value()));
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
fmin QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                       const quantity<Unit,Y>& q2)
{
    using namespace detail;
    ...
    return quantity_type::from_value(fmin QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value()));
}

template<class Unit,class Y>
inline 
int 
fpclassify QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{ 
    using qsboost::math::fpclassify;

    return fpclassify QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value());
}
...
typename root_typeof_helper<
    typename add_typeof_helper<
        ...
        static_rational<2> >::type
hypot QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,const quantity<Unit,Y>& q2)
{
    using qsboost::math::hypot;
    ...
    return quantity_type::from_value(hypot QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value()));
}
...
inline 
quantity<Unit,Y> 
nearbyint QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using namespace detail;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(nearbyint QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}
...
template<class Unit,class Y>
inline 
quantity<Unit,Y> nextafter QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                                             const quantity<Unit,Y>& q2)
{
    using qsboost::math::nextafter;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(nextafter QSBOOST_PREVENT_MACRO_SUBSTITUTION (q1.value(),q2.value()));
}
template<class Unit,class Y>
inline 
quantity<Unit,Y> nexttoward QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q1,
                                                              const quantity<Unit,Y>& q2)
{
...
inline 
quantity<Unit,Y> 
rint QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using namespace detail;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(rint QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}
...
inline 
quantity<Unit,Y> 
round QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using qsboost::math::round;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(round QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}

template<class Unit,class Y>
inline 
int 
signbit QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{ 
    using qsboost::math::signbit;

    return signbit QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value());
}

template<class Unit,class Y>
inline 
quantity<Unit,Y> 
trunc QSBOOST_PREVENT_MACRO_SUBSTITUTION (const quantity<Unit,Y>& q)
{
    using namespace detail;

    typedef quantity<Unit,Y>    quantity_type;
    
    return quantity_type::from_value(trunc QSBOOST_PREVENT_MACRO_SUBSTITUTION (q.value()));
}
...
inline
quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>
pow(const quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>& q1,
    const quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>& q2)
{
    using std::pow;

    typedef quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S),Y> quantity_type;
...
inline
quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>
exp(const quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>& q)
{
    using std::exp;
...
inline
quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>
log(const quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>& q)
{
    using std::log;
...
inline
quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>
log10(const quantity<QSBOOST_UNITS_DIMENSIONLESS_UNIT(S), Y>& q)
{
    using std::log10;
