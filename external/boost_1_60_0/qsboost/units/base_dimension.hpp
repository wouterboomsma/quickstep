...
#ifndef QSBOOST_UNITS_BASE_DIMENSION_HPP
#define QSBOOST_UNITS_BASE_DIMENSION_HPP

#include <qsboost/units/config.hpp>
#include <qsboost/units/dim.hpp>
...
struct check_base_dimension {
    enum {
        value = 
            sizeof(qsboost_units_is_registered(units::base_dimension_ordinal<N>())) == sizeof(detail::yes) &&
            sizeof(qsboost_units_is_registered(units::base_dimension_pair<T, N>())) != sizeof(detail::yes)
    };
};
    ...
    public ordinal<N> 
{
    ...
    private:
        ...
        /// INTERNAL ONLY
        friend detail::yes 
        qsboost_units_is_registered(const units::base_dimension_ordinal<N>&) 
        { detail::yes result; return(result); }
        
        /// But make sure we can identify the current instantiation!
        /// INTERNAL ONLY
        friend detail::yes 
        qsboost_units_is_registered(const units::base_dimension_pair<Derived, N>&) 
        { detail::yes result; return(result); }
};
