...
#ifndef QSBOOST_UNITS_CONVERSION_HPP
#define QSBOOST_UNITS_CONVERSION_HPP

/// \file
/// \brief Template for defining conversions between quantities.

#include <qsboost/units/detail/conversion_impl.hpp>
...
#define QSBOOST_UNITS_DEFINE_CONVERSION_FACTOR(Source, Destination, type_, value_)    \
    namespace qsboost {                                                       \
    namespace units {                                                       \
    template<>                                                              \
    ...
    void qsboost_units_require_semicolon()
...
#define QSBOOST_UNITS_DEFINE_CONVERSION_FACTOR_TEMPLATE(Params, Source, Destination, type_, value_)   \
    namespace qsboost {                                                       \
    namespace units {                                                       \
    template<QSBOOST_PP_SEQ_ENUM(Params)>                                     \
    struct base_unit_converter<                                             \
        Source,                                                             \
        QSBOOST_UNITS_MAKE_HETEROGENEOUS_UNIT(Destination, typename Source::dimension_type)\
    >                                                                       \
    {                                                                       \
        static const bool is_defined = true;                                \
        typedef type_ type;                                                 \
...
/// same dimensions.
#define QSBOOST_UNITS_DEFAULT_CONVERSION(Source, Dest)                \
    namespace qsboost {                                               \
    namespace units {                                               \
    template<>                                                      \
...
/// base unit.
#define QSBOOST_UNITS_DEFAULT_CONVERSION_TEMPLATE(Params, Source, Dest)   \
    namespace qsboost {                                                   \
    namespace units {                                                   \
    template<QSBOOST_PP_SEQ_ENUM(Params)>                                 \
...
#define QSBOOST_UNITS_DEFINE_BASE_UNIT_WITH_CONVERSIONS(namespace_, name_, name_string_, symbol_string_, factor, unit, id)\
namespace qsboost {                                                           \
namespace units {                                                           \
...
QSBOOST_UNITS_DEFINE_CONVERSION_FACTOR(namespace_::name_ ## _base_unit, unit, double, factor); \
QSBOOST_UNITS_DEFAULT_CONVERSION(namespace_::name_ ## _base_unit, unit)
