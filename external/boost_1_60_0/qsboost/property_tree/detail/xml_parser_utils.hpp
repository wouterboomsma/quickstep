...
// ----------------------------------------------------------------------------
#ifndef QSBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
#define QSBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_UTILS_HPP_INCLUDED
	...
	typedef typename Str::value_type Ch;
        ...
        for (typename Str::const_iterator it = s.begin(); it != end; ++it)
        {
            if (*it == Ch('&'))
            {
                ...
                if (semicolon == end)
                    QSBOOST_PROPERTY_TREE_THROW(xml_parser_error("invalid character entity", "", 0));
