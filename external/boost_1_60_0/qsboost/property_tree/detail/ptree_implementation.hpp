...
// ----------------------------------------------------------------------------
#ifndef QSBOOST_PROPERTY_TREE_DETAIL_PTREE_IMPLEMENTATION_HPP_INCLUDED
#define QSBOOST_PROPERTY_TREE_DETAIL_PTREE_IMPLEMENTATION_HPP_INCLUDED

#include <qsboost/iterator/iterator_adaptor.hpp>
#include <qsboost/iterator/reverse_iterator.hpp>
#include <qsboost/assert.hpp>
#include <qsboost/utility/swap.hpp>
#include <memory>

#if (defined(QSBOOST_MSVC) && \
     (_MSC_FULL_VER >= 160000000 && _MSC_FULL_VER < 170000000)) || \
    (defined(QSBOOST_INTEL_WIN) && \
     defined(QSBOOST_DINKUMWARE_STDLIB))
#define QSBOOST_PROPERTY_TREE_PAIR_BUG
#endif

namespace qsboost { namespace property_tree
{
    ...
    struct basic_ptree<K, D, C>::subs
    {
        ...
        static by_name_index& assoc(self_type *s) {
            return ch(s).QSBOOST_NESTED_TEMPLATE get<by_name>();
        }
    ...
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::iterator : public qsboost::iterator_adaptor<
        iterator, typename subs::base_container::iterator, value_type>
    {
        friend class qsboost::iterator_core_access;
        typedef qsboost::iterator_adaptor<
            iterator, typename subs::base_container::iterator, value_type>
            baset;
    public:
    ...
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::const_iterator : public qsboost::iterator_adaptor<
    ...
    class basic_ptree<K, D, C>::reverse_iterator
        : public qsboost::reverse_iterator<iterator>
    {
    public:
        reverse_iterator() {}
        explicit reverse_iterator(iterator b)
            : qsboost::reverse_iterator<iterator>(b)
        {}
    };
    template <class K, class D, class C>
    class basic_ptree<K, D, C>::const_reverse_iterator
        : public qsboost::reverse_iterator<const_iterator>
    {
    public:
        const_reverse_iterator() {}
        explicit const_reverse_iterator(const_iterator b)
            : qsboost::reverse_iterator<const_iterator>(b)
        {}
        const_reverse_iterator(
    ...
    class basic_ptree<K, D, C>::assoc_iterator
        : public qsboost::iterator_adaptor<assoc_iterator,
                                         typename subs::by_name_index::iterator,
                                         value_type>
    {
        friend class qsboost::iterator_core_access;
        typedef qsboost::iterator_adaptor<assoc_iterator,
                                         typename subs::by_name_index::iterator,
                                         value_type>
            baset;
    public:
    ...
    class basic_ptree<K, D, C>::const_assoc_iterator
        : public qsboost::iterator_adaptor<const_assoc_iterator,
    ...
    {
        qsboost::swap(m_data, rhs.m_data);
        ...
        basic_ptree<K, D, C>::to_iterator(assoc_iterator ai)
    {
        return iterator(subs::ch(this).
            QSBOOST_NESTED_TEMPLATE project<0>(ai.base()));
    }
        ...
        basic_ptree<K, D, C>::get_child(const path_type &path)
    {
        ...
        if (!n) {
            QSBOOST_PROPERTY_TREE_THROW(ptree_bad_path("No such node", path));
        }
        return *n;
    }
    ...
    typename qsboost::enable_if<detail::is_translator<Translator>, Type>::type
    basic_ptree<K, D, C>::get_value(Translator tr) const
    {
        if(qsboost::optional<Type> o = get_value_optional<Type>(tr)) {
            return *o;
        }
        QSBOOST_PROPERTY_TREE_THROW(ptree_bad_data(
    ...
    template<class K, class D, class C>
    template<class Type> inline
    typename qsboost::disable_if<detail::is_translator<Type>, Type>::type
    ...
    basic_ptree<K, D, C>::get(const path_type &path,
                              Translator tr) const
    {
        return get_child(path).QSBOOST_NESTED_TEMPLATE get_value<Type>(tr);
    }
    ...
    {
        return get_child(path).QSBOOST_NESTED_TEMPLATE get_value<Type>();
    }
                                                         ...
                                                         Translator tr) const
    {
        if (optional<const self_type&> child = get_child_optional(path))
            return child.get().
                QSBOOST_NESTED_TEMPLATE get_value_optional<Type>(tr);
        else
    ...
    {
        if (optional<const self_type&> child = get_child_optional(path))
            return child.get().QSBOOST_NESTED_TEMPLATE get_value_optional<Type>();
        else
            return optional<Type>();
    }
    ...
    {
        QSBOOST_ASSERT(!p.empty() && "Empty path not allowed for put_child.");
        if(p.single()) {
...
#if defined(QSBOOST_PROPERTY_TREE_PAIR_BUG)
#undef QSBOOST_PROPERTY_TREE_PAIR_BUG
#endif
