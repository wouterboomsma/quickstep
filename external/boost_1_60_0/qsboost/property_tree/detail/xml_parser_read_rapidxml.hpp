...
// ----------------------------------------------------------------------------
#ifndef QSBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_READ_RAPIDXML_HPP_INCLUDED
#define QSBOOST_PROPERTY_TREE_DETAIL_XML_PARSER_READ_RAPIDXML_HPP_INCLUDED

#include <qsboost/property_tree/ptree.hpp>
#include <qsboost/property_tree/detail/xml_parser_error.hpp>
#include <qsboost/property_tree/detail/xml_parser_flags.hpp>
#include <qsboost/property_tree/detail/xml_parser_utils.hpp>
#include <qsboost/property_tree/detail/rapidxml.hpp>
#include <vector>

namespace qsboost { namespace property_tree { namespace xml_parser
{
        ...
        try {
            ...
            if (flags & no_comments) {
                if (flags & trim_whitespace)
                    doc.QSBOOST_NESTED_TEMPLATE parse<f_tws>(&v.front());
                else
                    doc.QSBOOST_NESTED_TEMPLATE parse<0>(&v.front());
            } else {
                if (flags & trim_whitespace)
                    doc.QSBOOST_NESTED_TEMPLATE parse<f_tws_c>(&v.front());
                else
                    doc.QSBOOST_NESTED_TEMPLATE parse<f_c>(&v.front());
            }
