#ifndef QSBOOST_ARCHIVE_ARCHIVE_EXCEPTION_HPP
#define QSBOOST_ARCHIVE_ARCHIVE_EXCEPTION_HPP
...
#include <qsboost/archive/detail/decl.hpp>
...
#if defined(QSBOOST_WINDOWS) 
#include <excpt.h> 
#endif 

#include <qsboost/archive/detail/abi_prefix.hpp> // must be the last header
...
//
class QSBOOST_SYMBOL_VISIBLE archive_exception : 
    public virtual std::exception
{
private:
    char m_buffer[128];
protected:
    QSBOOST_ARCHIVE_DECL unsigned int
    append(unsigned int l, const char * a);
    QSBOOST_ARCHIVE_DECL
    archive_exception() QSBOOST_NOEXCEPT;
public:
    typedef enum {
    ...
    QSBOOST_ARCHIVE_DECL archive_exception(
        exception_code c, 
        const char * e1 = NULL,
        const char * e2 = NULL
    ) QSBOOST_NOEXCEPT;
    QSBOOST_ARCHIVE_DECL archive_exception(archive_exception const &) QSBOOST_NOEXCEPT ;
    virtual QSBOOST_ARCHIVE_DECL ~archive_exception() QSBOOST_NOEXCEPT_OR_NOTHROW ;
    virtual QSBOOST_ARCHIVE_DECL const char * what() const QSBOOST_NOEXCEPT_OR_NOTHROW ;
};

}// namespace archive
}// namespace boost

#include <qsboost/archive/detail/abi_suffix.hpp> // pops abi_suffix.hpp pragmas
