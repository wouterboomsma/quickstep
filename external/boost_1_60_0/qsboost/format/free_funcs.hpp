...
#ifndef QSBOOST_FORMAT_FUNCS_HPP
#define QSBOOST_FORMAT_FUNCS_HPP
...
namespace qsboost {
    ...
    namespace io {
         using ::qsboost::str; // keep compatibility with when it was defined in this N.S.
    }   // - namespace io
    ...
    {
        typedef qsboost::basic_format<Ch, Tr, Alloc>   format_t;
        if(f.items_.size()==0) 
            os << f.prefix_;
        else {
            if(f.cur_arg_ < f.num_args_)
                if( f.exceptions() & io::too_few_args_bit )
                    ...
                    qsboost::throw_exception(io::too_few_args(f.cur_arg_, f.num_args_)); 
