...
#ifndef QSBOOST_FT_DETAIL_CV_TRAITS_HPP_INCLUDED
#define QSBOOST_FT_DETAIL_CV_TRAITS_HPP_INCLUDED

#include <cstddef>
#include <qsboost/detail/workaround.hpp>

#if QSBOOST_WORKAROUND(__BORLANDC__, <= 0x582)
...
struct cv_code
{
  ...
  QSBOOST_STATIC_CONSTANT(std::size_t, value = 
      sizeof(::qsboost::function_types::detail::switch_cv(
         ::qsboost::function_types::detail::ref_to_ptr(_t) ) ));
};

template<typename T> struct cv_traits 
{
  typedef typename qsboost::function_types::detail::cv_tag_impl< 
    ::qsboost::function_types::detail::cv_code<T>::value >::type
  tag;

  // may require Boost.TypeTraits broken compiler specializations
  // to work
  typedef typename qsboost::remove_cv<
              typename qsboost::remove_pointer<
                  typename qsboost::remove_reference<T>::type 
              >::type 
          >::type type; 
};
#endif
