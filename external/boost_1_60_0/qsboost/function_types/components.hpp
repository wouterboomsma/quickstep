...
#ifndef QSBOOST_FT_COMPONENTS_HPP_INCLUDED
#define QSBOOST_FT_COMPONENTS_HPP_INCLUDED
...
#include <qsboost/mpl/vector/vector0.hpp>

#if QSBOOST_WORKAROUND(__BORLANDC__, <= 0x565)
...
#   include <qsboost/mpl/copy.hpp>
#   include <qsboost/mpl/front_inserter.hpp>

#   include <qsboost/function_types/detail/classifier.hpp>
#endif

#ifndef QSBOOST_FT_NO_CV_FUNC_SUPPORT
#   include <qsboost/mpl/remove.hpp>
#endif
...
#     include <qsboost/mpl/vector/vector20.hpp>
#   elif QSBOOST_FT_MAX_ARITY < 30
#     include <qsboost/mpl/vector/vector30.hpp>
#   elif QSBOOST_FT_MAX_ARITY < 40
#     include <qsboost/mpl/vector/vector40.hpp>
#   elif QSBOOST_FT_MAX_ARITY < 50
#     include <qsboost/mpl/vector/vector50.hpp>
#   endif

#include <qsboost/function_types/detail/class_transform.hpp>
...
{ 
  namespace function_types 
  {
    ...
    { 
      ...
      QSBOOST_MPL_AUX_LAMBDA_SUPPORT(2,components,(T,ClassTypeTransform))
    };
...
#if !QSBOOST_FT_NO_CV_FUNC_SUPPORT
...
#if !QSBOOST_WORKAROUND(__BORLANDC__, <= 0x565)
#   define QSBOOST_FT_variations QSBOOST_FT_pointer|QSBOOST_FT_member_pointer
...
#else  
#   define QSBOOST_FT_variations QSBOOST_FT_pointer
    ...
    struct components_impl<T C::*, L>
      : detail::retagged_if
        < detail::components_impl<typename qsboost::remove_cv<T>::type *, L>
    ...
    { typedef char (& type)[1]; QSBOOST_STATIC_CONSTANT(std::size_t, value = 1); };
    template<typename T> struct encode_cv<T const *>
    { typedef char (& type)[2]; QSBOOST_STATIC_CONSTANT(std::size_t, value = 2); };
    template<typename T> struct encode_cv<T volatile *>
    { typedef char (& type)[3]; QSBOOST_STATIC_CONSTANT(std::size_t, value = 3); };
    template<typename T> struct encode_cv<T const volatile *> 
    { typedef char (& type)[4]; QSBOOST_STATIC_CONSTANT(std::size_t, value = 4); };
    ...
    { 
      QSBOOST_STATIC_CONSTANT(std::size_t, value = 
    ...
    template<typename T> struct cv_tag_mfp
      : detail::cv_tag_mfp_impl
        < ::qsboost::function_types::detail::encode_mfp_cv<T>::value >
    { };
    ...
    template<typename T, typename D, typename L> struct bcc_class_transform 
      : bcc_class_transform_impl
        < typename decode_cv
          < T
          , ::qsboost::function_types::detail::encode_mfp_cv<D>::value 
          >::type
        , L
        > 
    { };
...
#define QSBOOST_FT_al_path qsboost/function_types/detail/components_impl
...
#include <qsboost/function_types/detail/components_as_mpl_sequence.hpp>
