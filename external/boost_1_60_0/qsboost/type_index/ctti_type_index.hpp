...
#ifndef QSBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP
#define QSBOOST_TYPE_INDEX_CTTI_TYPE_INDEX_HPP
...
#include <qsboost/type_index/detail/compile_time_type_info.hpp>
...
/// \endcode
class ctti_data {
#ifndef QSBOOST_NO_CXX11_DELETED_FUNCTIONS
public:
    ctti_data() = delete;
...
template <class T>
inline const detail::ctti_data& ctti_construct() QSBOOST_NOEXCEPT {
    ...
    return *reinterpret_cast<const detail::ctti_data*>(qsboost::detail::ctti<T>::n());
}
...
class ctti_type_index: public type_index_facade<ctti_type_index, detail::ctti_data> {
    ...
    inline std::size_t get_raw_name_length() const QSBOOST_NOEXCEPT;

public:
    typedef detail::ctti_data type_info_t;

    inline ctti_type_index() QSBOOST_NOEXCEPT
        : data_(&ctti_construct<void>())
    {}

    inline ctti_type_index(const type_info_t& data) QSBOOST_NOEXCEPT
        : data_(&data)
    {}
    ...
    template <class T>
    inline static ctti_type_index type_id() QSBOOST_NOEXCEPT;

    template <class T>
    inline static ctti_type_index type_id_with_cvr() QSBOOST_NOEXCEPT;

    template <class T>
    inline static ctti_type_index type_id_runtime(const T& variable) QSBOOST_NOEXCEPT;
};


inline const ctti_type_index::type_info_t& ctti_type_index::type_info() const QSBOOST_NOEXCEPT {
    return *data_;
}


template <class T>
inline ctti_type_index ctti_type_index::type_id() QSBOOST_NOEXCEPT {
    typedef QSBOOST_DEDUCED_TYPENAME qsboost::remove_reference<T>::type no_ref_t;
    typedef QSBOOST_DEDUCED_TYPENAME qsboost::remove_cv<no_ref_t>::type no_cvr_t;
...
template <class T>
inline ctti_type_index ctti_type_index::type_id_with_cvr() QSBOOST_NOEXCEPT {
    return ctti_construct<T>();
}


template <class T>
inline ctti_type_index ctti_type_index::type_id_runtime(const T& variable) QSBOOST_NOEXCEPT {
    return variable.qsboost_type_index_type_id_runtime_();
}


inline const char* ctti_type_index::raw_name() const QSBOOST_NOEXCEPT {
    return reinterpret_cast<const char*>(data_);
}

inline std::size_t ctti_type_index::get_raw_name_length() const QSBOOST_NOEXCEPT {
...
inline std::size_t ctti_type_index::hash_code() const QSBOOST_NOEXCEPT {
    return qsboost::hash_range(raw_name(), raw_name() + get_raw_name_length());
}
