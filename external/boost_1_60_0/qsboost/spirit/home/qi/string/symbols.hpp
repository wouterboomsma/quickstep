...
==============================================================================*/
#if !defined(QSBOOST_SPIRIT_SYMBOLS_MARCH_11_2007_1055AM)
#define QSBOOST_SPIRIT_SYMBOLS_MARCH_11_2007_1055AM
...
#include <qsboost/spirit/home/qi/string/tst.hpp>
...
namespace qsboost { namespace spirit { namespace qi
{
      ...
      , primitive_parser<symbols<Char, T, Lookup, Filter> >
    {
        ...
        symbols(Symbols const& syms, std::string const& name = "symbols")
          ...
          , name_(name)
        {
            typename range_const_iterator<Symbols>::type si = qsboost::begin(syms);
            while (si != qsboost::end(syms))
                add(*si++);
        }
        ...
        {
            typename range_const_iterator<Symbols>::type si = qsboost::begin(syms);
            typename range_const_iterator<Data>::type di = qsboost::begin(data);
            while (si != qsboost::end(syms))
                add(*si++, *di++);
        }
