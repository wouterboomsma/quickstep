...
==============================================================================*/
#if !defined(QSBOOST_SPIRIT_SIMPLE_TRACE_DECEMBER_06_2008_1102AM)
#define QSBOOST_SPIRIT_SIMPLE_TRACE_DECEMBER_06_2008_1102AM
...
#if !defined(QSBOOST_SPIRIT_DEBUG_OUT)
#define QSBOOST_SPIRIT_DEBUG_OUT std::cerr
#endif

//  number of tokens to print while debugging
#if !defined(QSBOOST_SPIRIT_DEBUG_PRINT_SOME)
#define QSBOOST_SPIRIT_DEBUG_PRINT_SOME 20
#endif

//  number of spaces to indent
#if !defined(QSBOOST_SPIRIT_DEBUG_INDENT)
#define QSBOOST_SPIRIT_DEBUG_INDENT 2
#endif
...
namespace qsboost { namespace spirit { namespace qi
{
    ...
    struct simple_trace
    {
        ...
        {
            n *= QSBOOST_SPIRIT_DEBUG_INDENT;
            for (int i = 0; i != n; ++i)
                QSBOOST_SPIRIT_DEBUG_OUT << ' ';
        }
          ...
          , Iterator first, Iterator const& last) const
        {
            ...
            QSBOOST_SPIRIT_DEBUG_OUT << '<' << tag << '>';
            int const n = QSBOOST_SPIRIT_DEBUG_PRINT_SOME;
            for (int i = 0; first != last && i != n && *first; ++i, ++first)
                detail::token_printer(QSBOOST_SPIRIT_DEBUG_OUT, *first);
            QSBOOST_SPIRIT_DEBUG_OUT << "</" << tag << '>' << std::endl;
          ...
          , std::string const& rule_name) const
        {
            switch (state)
            {
                ...
                case successful_parse:
                    ...
                    if (!fusion::empty(context.locals))
                        QSBOOST_SPIRIT_DEBUG_OUT
                            ...
                            << context.locals
                            << "</locals>";
                    QSBOOST_SPIRIT_DEBUG_OUT << std::endl;
                    print_indent(--get_indent());
                    QSBOOST_SPIRIT_DEBUG_OUT
                        << "</" << rule_name << '>'
                        << std::endl;
                ...
                case failed_parse:
                    print_indent(get_indent());
                    QSBOOST_SPIRIT_DEBUG_OUT << "<fail/>" << std::endl;
                    print_indent(--get_indent());
                    QSBOOST_SPIRIT_DEBUG_OUT
                        << "</" << rule_name << '>'
                        << std::endl;
