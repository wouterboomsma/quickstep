...
==============================================================================*/
#if !defined(QSBOOST_SPIRIT_HOLD_ANY_MAY_02_2007_0857AM)
#define QSBOOST_SPIRIT_HOLD_ANY_MAY_02_2007_0857AM
...
namespace qsboost { namespace spirit
{
    struct bad_any_cast
      : std::bad_cast
    {
        bad_any_cast(qsboost::detail::sp_typeinfo const& src, qsboost::detail::sp_typeinfo const& dest)
    ...
    namespace detail
    {
        ...
        {
            qsboost::detail::sp_typeinfo const& (*get_type)();
        ...
        struct fxns<mpl::true_>
        {
            ...
            struct type
            {
                static qsboost::detail::sp_typeinfo const& get_type()
                {
                    return QSBOOST_SP_TYPEID(T);
                }
    ...
    public:
        ...
        qsboost::detail::sp_typeinfo const& type() const
        {
        ...
        T const& cast() const
        {
            if (type() != QSBOOST_SP_TYPEID(T))
              throw bad_any_cast(type(), QSBOOST_SP_TYPEID(T));
...
// implicit casting is disabled by default for compatibility with boost::any
#ifdef QSBOOST_SPIRIT_ANY_IMPLICIT_CASTING
    ...
    {
        if (operand && operand->type() == QSBOOST_SP_TYPEID(T)) {
            return spirit::detail::get_table<T>::is_small::value ?
    ...
    {
        typedef QSBOOST_DEDUCED_TYPENAME remove_reference<T>::type nonref;


        nonref* result = any_cast<nonref>(&operand);
        if(!result)
            qsboost::throw_exception(bad_any_cast(operand.type(), QSBOOST_SP_TYPEID(T)));
        return *result;
    }
