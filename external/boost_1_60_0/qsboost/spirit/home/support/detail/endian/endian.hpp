...
#ifndef QSBOOST_SPIRIT_ENDIAN_HPP
#define QSBOOST_SPIRIT_ENDIAN_HPP

#if defined(_MSC_VER)
#pragma once
#endif

#ifdef QSBOOST_ENDIAN_LOG
# include <iostream>
#endif
...
#define QSBOOST_MINIMAL_INTEGER_COVER_OPERATORS
#define QSBOOST_NO_IO_COVER_OPERATORS
#include <qsboost/spirit/home/support/detail/endian/cover_operators.hpp>
#undef  QSBOOST_NO_IO_COVER_OPERATORS
#undef  QSBOOST_MINIMAL_INTEGER_COVER_OPERATORS
...
# define QSBOOST_ENDIAN_DEFAULT_CONSTRUCT {}          // C++03

# if defined(QSBOOST_ENDIAN_FORCE_PODNESS)
#   define QSBOOST_ENDIAN_NO_CTORS
# endif
...
namespace qsboost { namespace spirit
{
  namespace detail
  {
    ...
    template <typename T, std::size_t n_bytes,
      bool sign=qsboost::is_signed<T>::value >
    struct unrolled_byte_loops
    {
      typedef unrolled_byte_loops<T, n_bytes - 1, sign> next;

      static typename qsboost::make_unsigned<T>::type load_big(const unsigned char* bytes)
        { return *(bytes - 1) | (next::load_big(bytes - 1) << 8); }
      static typename qsboost::make_unsigned<T>::type load_little(const unsigned char* bytes)
...
# endif
  ...
  QSBOOST_SCOPED_ENUM_START(endianness) { big, little, native }; QSBOOST_SCOPED_ENUM_END
  QSBOOST_SCOPED_ENUM_START(alignment) { unaligned, aligned }; QSBOOST_SCOPED_ENUM_END

  template <QSBOOST_SCOPED_ENUM(endianness) E, typename T, std::size_t n_bits,
    QSBOOST_SCOPED_ENUM(alignment) A = alignment::unaligned>
    class endian;
    ...
    {
        QSBOOST_STATIC_ASSERT( (n_bits/8)*8 == n_bits );
      public:
        typedef T value_type;
#     ifndef QSBOOST_ENDIAN_NO_CTORS
        endian() QSBOOST_ENDIAN_DEFAULT_CONSTRUCT
...
#   endif
    ...
    {
        QSBOOST_STATIC_ASSERT( (n_bits/8)*8 == n_bits );
        QSBOOST_STATIC_ASSERT( sizeof(T) == n_bits/8 );
      public:
        typedef T value_type;
#   ifndef QSBOOST_ENDIAN_NO_CTORS
        endian() QSBOOST_ENDIAN_DEFAULT_CONSTRUCT
...
#define QSBOOST_HAS_INT16_T
#define QSBOOST_HAS_INT32_T
#define QSBOOST_HAS_INT64_T
...
# if defined(QSBOOST_HAS_INT16_T)
...
# if defined(QSBOOST_HAS_INT32_T)
...
# if defined(QSBOOST_HAS_INT64_T)
...
namespace qsboost { namespace endian
{
    using namespace qsboost::spirit::endian;
}}
