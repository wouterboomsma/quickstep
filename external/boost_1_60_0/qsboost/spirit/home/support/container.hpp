...
=============================================================================*/
#if !defined(QSBOOST_SPIRIT_CONTAINER_FEBRUARY_06_2007_1001AM)
#define QSBOOST_SPIRIT_CONTAINER_FEBRUARY_06_2007_1001AM
...
#include <qsboost/detail/iterator.hpp> // for boost::detail::iterator_traits
#include <qsboost/mpl/has_xxx.hpp>
#include <qsboost/mpl/bool.hpp>
#include <qsboost/optional.hpp>
...
namespace qsboost { namespace spirit { namespace traits
{
    ...
    namespace detail
    {
        QSBOOST_MPL_HAS_XXX_TRAIT_DEF(value_type)
        QSBOOST_MPL_HAS_XXX_TRAIT_DEF(iterator)
        QSBOOST_MPL_HAS_XXX_TRAIT_DEF(size_type)
        QSBOOST_MPL_HAS_XXX_TRAIT_DEF(reference)
    }
    ...
    template <typename T>
    struct is_container<qsboost::optional<T> >
      : is_container<T>
    {};

#if !defined(QSBOOST_VARIANT_DO_NOT_USE_VARIADIC_TEMPLATES)
    template<typename T>
    struct is_container<qsboost::variant<T> >
      : is_container<T>
    {};

    template<typename T0, typename T1, typename ...TN>
    struct is_container<qsboost::variant<T0, T1, TN...> >
      : mpl::bool_<is_container<T0>::value ||
            is_container<qsboost::variant<T1, TN...> >::value>
    {};

#else
#define QSBOOST_SPIRIT_IS_CONTAINER(z, N, data)                                 \
        is_container<QSBOOST_PP_CAT(T, N)>::value ||                            \
    /***/

    // make sure unused variant parameters do not affect the outcome
    template <>
    struct is_container<qsboost::detail::variant::void_>
      : mpl::false_
    {};

    template <QSBOOST_VARIANT_ENUM_PARAMS(typename T)>
    struct is_container<variant<QSBOOST_VARIANT_ENUM_PARAMS(T)> >
       : mpl::bool_<QSBOOST_PP_REPEAT(QSBOOST_VARIANT_LIMIT_TYPES
            , QSBOOST_SPIRIT_IS_CONTAINER, _) false>
    {};

#undef QSBOOST_SPIRIT_IS_CONTAINER
#endif
    ...
    template <typename T>
    struct container_value<qsboost::optional<T> >
      : container_value<T>
    {};

    // this will be instantiated if the variant holds a container
    template <QSBOOST_VARIANT_ENUM_PARAMS(typename T)>
    struct container_value<variant<QSBOOST_VARIANT_ENUM_PARAMS(T)> >
    {
        typedef typename
            variant<QSBOOST_VARIANT_ENUM_PARAMS(T)>::types
        types;
        typedef typename
    ...
    template <typename T>
    struct optional_attribute<qsboost::optional<T> >
    {
        typedef T const& type;

        static type call(qsboost::optional<T> const& val)
        {
            return qsboost::get<T>(val);
        }

        static bool is_valid(qsboost::optional<T> const& val)
        {
            return !!val;
        }
    };
    ...
    {
        static bool call(qsboost::optional<Container>& c, T const& val)
        {
            if (!c)
                c = Container();
            return push_back(qsboost::get<Container>(c), val);
        }
    };
    ...
    namespace detail
    {
        ...
        struct push_back_visitor : public static_visitor<>
        {
            ...
            bool push_back_impl(T_&, mpl::false_) const
            {
                ...
                QSBOOST_ASSERT(false && "This variant doesn't hold a container");
                return false;
            }
    ...
    template <QSBOOST_VARIANT_ENUM_PARAMS(typename T_), typename T>
    struct push_back_container<variant<QSBOOST_VARIANT_ENUM_PARAMS(T_)>, T>
    {
        static bool call(variant<QSBOOST_VARIANT_ENUM_PARAMS(T_)>& c, T const& val)
        {
...
namespace qsboost { namespace spirit { namespace result_of
{
    ...
    template <typename T>
    struct optional_value<qsboost::optional<T> >
    {
        typedef T type;
    };

    template <typename T>
    struct optional_value<qsboost::optional<T> const>
    {
        typedef T const type;
    };
