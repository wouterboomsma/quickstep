...
#ifndef QSBOOST_VARIANT_RECURSIVE_WRAPPER_HPP
#define QSBOOST_VARIANT_RECURSIVE_WRAPPER_HPP
...
public: // modifiers
    ...
    void swap(recursive_wrapper& operand) QSBOOST_NOEXCEPT
    {
        T* temp = operand.p_;
        operand.p_ = p_;
        p_ = temp;
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES 
    recursive_wrapper& operator=(recursive_wrapper&& rhs) QSBOOST_NOEXCEPT
    {
        swap(rhs);
        return *this;
    }
...
//
template <typename T>
inline void swap(recursive_wrapper<T>& lhs, recursive_wrapper<T>& rhs) QSBOOST_NOEXCEPT
{
    lhs.swap(rhs);
}
