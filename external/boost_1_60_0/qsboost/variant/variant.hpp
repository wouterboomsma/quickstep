...
#ifndef QSBOOST_VARIANT_VARIANT_HPP
#define QSBOOST_VARIANT_VARIANT_HPP
...
#include "qsboost/variant/detail/config.hpp"
#include "qsboost/mpl/aux_/value_wknd.hpp"

#include "qsboost/variant/variant_fwd.hpp"
#include "qsboost/variant/detail/backup_holder.hpp"
#include "qsboost/variant/detail/enable_recursive_fwd.hpp"
#include "qsboost/variant/detail/forced_return.hpp"
#include "qsboost/variant/detail/initializer.hpp"
#include "qsboost/variant/detail/make_variant_list.hpp"
#include "qsboost/variant/detail/over_sequence.hpp"
#include "qsboost/variant/detail/visitation_impl.hpp"
#include "qsboost/variant/detail/hash_variant.hpp"
...
#include "qsboost/math/common_factor_ct.hpp"
...
#include "qsboost/mpl/max_element.hpp"
...
#include "qsboost/mpl/sizeof.hpp"
...
#if defined(QSBOOST_VARIANT_MINIMIZE_SIZE)
...
struct add_alignment
{
    ...
    struct apply
        : mpl::size_t<
              ::qsboost::math::static_lcm<
                  QSBOOST_MPL_AUX_VALUE_WKND(State)::value
                , ::qsboost::alignment_of<Item>::value
                >::value
            >
    {};
};
...
private: // helpers, for metafunction result (below)
    ...
    typedef typename mpl::iter_fold_if<
        ...
        , mpl::protect< mpl::not_same_as<qsboost::blank> >
        >::type second_result_;
...
struct is_variant_move_noexcept_constructible {
    typedef typename qsboost::mpl::find_if<
        Types, mpl::not_<qsboost::is_nothrow_move_constructible<qsboost::mpl::_1> >
    >::type iterator_t;

    typedef typename qsboost::mpl::end<Types>::type end_t;
    typedef typename qsboost::is_same<
        iterator_t, end_t
    >::type type;
};
...
struct is_variant_move_noexcept_assignable {
    typedef typename qsboost::mpl::find_if<
        Types, mpl::not_<qsboost::is_nothrow_move_assignable<qsboost::mpl::_1> >
    >::type iterator_t;
...
public: // metafunction result
    ...
    typedef ::qsboost::aligned_storage<
          QSBOOST_MPL_AUX_VALUE_WKND(max_size)::value
        , QSBOOST_MPL_AUX_VALUE_WKND(max_alignment)::value
        > type;
};
...
public: // visitor interfaces
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(T& operand, int) const QSBOOST_NOEXCEPT
    {
        operand.~T(); // must be noexcept

#if QSBOOST_WORKAROUND(__BORLANDC__, QSBOOST_TESTED_AT(0x0551)) || \
    QSBOOST_WORKAROUND(QSBOOST_MSVC, QSBOOST_TESTED_AT(1600))
        operand; // suppresses warnings
#endif

        QSBOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // visitor interface
    ...
    T& operator()(T& operand) const QSBOOST_NOEXCEPT
    {
        return operand;
    }
    ...
    T& operator()(U&) const
    {
        ...
        return ::qsboost::detail::variant::forced_return< T& >();
    }
};
...
public: // structors
    ...
    explicit copy_into(void* storage) QSBOOST_NOEXCEPT
        : storage_(storage)
    {
    }
...
public: // internal visitor interface
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(qsboost::detail::variant::backup_holder<T>& operand, long) const
    {
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(const qsboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( operand.get() );
        QSBOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // structors
    ...
    explicit move_into(void* storage) QSBOOST_NOEXCEPT
        : storage_(storage)
    {
    }
...
public: // internal visitor interface
    ...
    internal_visit(qsboost::detail::variant::backup_holder<T>& operand, long) const
    {
        new(storage_) T( ::qsboost::detail::variant::move(operand.get()) );
        QSBOOST_VARIANT_AUX_RETURN_VOID;
    }

    template <typename T>
        QSBOOST_VARIANT_AUX_RETURN_VOID_TYPE
    internal_visit(T& operand, int) const QSBOOST_NOEXCEPT_IF(QSBOOST_NOEXCEPT_EXPR(T(qsboost::declval<T>())))
    {
        new(storage_) T(::qsboost::detail::variant::move(operand));
        QSBOOST_VARIANT_AUX_RETURN_VOID;
    }
};
#endif
...
public: // structors
    ...
    explicit assign_storage(const void* rhs_storage) QSBOOST_NOEXCEPT
...
public: // structors
    ...
    explicit move_storage(void* rhs_storage) QSBOOST_NOEXCEPT
        : rhs_storage_(rhs_storage)
    {
    }
...
public: // internal visitor interfaces
    ...
    {
        lhs_content.get()
            = ::qsboost::detail::variant::move(static_cast<backup_holder<T>* >(rhs_storage_)->get());
        QSBOOST_VARIANT_AUX_RETURN_VOID;
    }
    ...
    internal_visit(T& lhs_content, int) const
    {
        ...
        // Hint: Are any of the bounded types const-qualified or references?
        //
        lhs_content = ::qsboost::detail::variant::move(*static_cast<T* >(rhs_storage_));
        QSBOOST_VARIANT_AUX_RETURN_VOID;
    }
...
public: // structors
    ...
    explicit direct_assigner(const T& rhs) QSBOOST_NOEXCEPT
        : rhs_(rhs)
    {
    }
...
public: // visitor interface
    ...
    template <typename U>
    bool operator()(U&) QSBOOST_NOEXCEPT
    {
        return false;
    }

#if QSBOOST_WORKAROUND(QSBOOST_MSVC, QSBOOST_TESTED_AT(1600))
private:
...
public: // structors
    ...
    explicit direct_mover(T& rhs) QSBOOST_NOEXCEPT
        : rhs_(rhs)
    {
    }
...
public: // visitor interface
    ...
    bool operator()(T& lhs)
    {
        lhs = ::qsboost::detail::variant::move(rhs_);
        return true;
    }
...
private: // helpers, for visitor interface (below)
        ...
        )
    {
        ...
        LhsT backup_lhs_content(
              ::qsboost::detail::variant::move(lhs_content)
            ); // nothrow
        ...
        QSBOOST_CATCH (...)
        {
            ...
            new(lhs_.storage_.address())
                LhsT(
                      ::qsboost::detail::variant::move(backup_lhs_content)
                    ); // nothrow
...
public: // structors
    ...
    explicit swap_with(Variant& toswap) QSBOOST_NOEXCEPT
        : toswap_(toswap)
    {
    }
...
public: // internal visitor interfaces
    ...
    void operator()(T& operand) const
    {
        ...
        // ...and swap:
        ::qsboost::detail::variant::move_swap( operand, other );
    }
...
class reflect
    : public static_visitor<const qsboost::typeindex::type_info&>
{
public: // visitor interfaces

    template <typename T>
    const qsboost::typeindex::type_info& operator()(const T&) const QSBOOST_NOEXCEPT
    {
        return qsboost::typeindex::type_id<T>().type_info();
    }
...
public: // structors
    ...
    explicit comparer(const Variant& lhs) QSBOOST_NOEXCEPT
        : lhs_(lhs)
    {
    }
...
public: // structors
    ...
    explicit invoke_visitor(Visitor& visitor) QSBOOST_NOEXCEPT
        : visitor_(visitor)
    {
    }
...
#   if QSBOOST_WORKAROUND(__BORLANDC__, QSBOOST_TESTED_AT(0x0564))
    template <typename T>
...
public: // internal visitor interfaces, cont.
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(qsboost::recursive_wrapper<T>& operand, long)
    {
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const qsboost::recursive_wrapper<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(qsboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const qsboost::detail::reference_content<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(qsboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
    ...
    template <typename T>
        QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(result_type)
    internal_visit(const qsboost::detail::variant::backup_holder<T>& operand, long)
    {
        return internal_visit( operand.get(), 1L );
    }
...
template <
      typename T0_
    , QSBOOST_VARIANT_ENUM_SHIFTED_PARAMS(typename T)
    >
class variant
{
...
#if !defined(QSBOOST_VARIANT_NO_TYPE_SEQUENCE_SUPPORT)
...
private: // helpers, for typedefs (below)
    ...
    QSBOOST_STATIC_ASSERT((
          ::qsboost::mpl::not_< mpl::empty<specified_types> >::value
        ));
...
private: // helpers, for typedefs (below)
    ...
    #define QSBOOST_VARIANT_AUX_ENABLE_RECURSIVE_TYPEDEFS(z,N,_) \
        typedef typename mpl::eval_if< \
            ...
            , detail::variant::enable_recursive< \
                ...
                , wknd_self_t \
                > \
            , mpl::identity< QSBOOST_PP_CAT(T,N) > \
            >::type QSBOOST_PP_CAT(recursive_enabled_T,N); \
        /**/
    ...
    #define QSBOOST_VARIANT_AUX_UNWRAP_RECURSIVE_TYPEDEFS(z,N,_) \
    ...
    QSBOOST_PP_REPEAT(
          QSBOOST_VARIANT_LIMIT_TYPES
        , QSBOOST_VARIANT_AUX_UNWRAP_RECURSIVE_TYPEDEFS
        , _
        )

    #undef QSBOOST_VARIANT_AUX_UNWRAP_RECURSIVE_TYPEDEFS

public: // public typedefs

    typedef typename detail::variant::make_variant_list<
          QSBOOST_VARIANT_ENUM_PARAMS(public_T)
        >::type types;

private: // helpers, for internal typedefs (below)

    #define QSBOOST_VARIANT_AUX_MAKE_REFERENCE_CONTENT_TYPEDEFS(z,N,_) \
        typedef detail::make_reference_content< \
              QSBOOST_PP_CAT(recursive_enabled_T,N) \
            >::type QSBOOST_PP_CAT(internal_T,N); \
        /**/

    QSBOOST_PP_REPEAT(
          QSBOOST_VARIANT_LIMIT_TYPES
        , QSBOOST_VARIANT_AUX_MAKE_REFERENCE_CONTENT_TYPEDEFS
        , _
        )

    #undef QSBOOST_VARIANT_AUX_MAKE_REFERENCE_CONTENT_TYPEDEFS

private: // internal typedefs

    typedef typename detail::variant::make_variant_list<
          QSBOOST_VARIANT_ENUM_PARAMS(internal_T)
        >::type internal_types;
...
private: // static precondition assertions
    ...
    //
    QSBOOST_MPL_ASSERT_NOT(( is_sequence_based_ ));
...
// representation -- private when possible
    ...
    void indicate_which(int which_arg) QSBOOST_NOEXCEPT
    {
        which_ = static_cast<which_t>( which_arg );
    }

    void indicate_backup_which(int which_arg) QSBOOST_NOEXCEPT
    {
...
private: // helpers, for queries (below)
    ...
    bool using_backup() const QSBOOST_NOEXCEPT
    {
        return which_ < 0;
    }
...
public: // queries
    ...
    int which() const QSBOOST_NOEXCEPT
    {
...
private: // helpers, for structors (below)
    ...
    struct initializer
        : QSBOOST_VARIANT_AUX_INITIALIZER_T(
              recursive_enabled_types, recursive_enabled_T
            )
    {
    };

    void destroy_content() QSBOOST_NOEXCEPT
    {
...
public: // structors
    ...
    ~variant() QSBOOST_NOEXCEPT
    {
        destroy_content();
    }

    variant() 
#if !(defined(__SUNPRO_CC) && QSBOOST_WORKAROUND(__SUNPRO_CC, <= 0x5130))
              QSBOOST_NOEXCEPT_IF(qsboost::has_nothrow_constructor<internal_T0>::value)
#endif
    {
#ifdef _MSC_VER
#pragma warning( push )
...
private: // helpers, for structors, cont. (below)
    ...
    public: // structors
        ...
        explicit convert_copy_into(void* storage) QSBOOST_NOEXCEPT
            : storage_(storage)
        {
        }
...
#   endif
        ...
        template <typename T>
        int internal_visit(qsboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const qsboost::detail::reference_content<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(qsboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const qsboost::detail::variant::backup_holder<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(qsboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }

        template <typename T>
        int internal_visit(const qsboost::recursive_wrapper<T>& operand, long) const
        {
            return internal_visit( operand.get(), 1L );
        }
    ...
    public: // structors
        ...
        explicit convert_move_into(void* storage) QSBOOST_NOEXCEPT
            : storage_(storage)
        {
        }
...
private: // helpers, for structors, below 
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename T>
    typename qsboost::enable_if<qsboost::is_rvalue_reference<T&&> >::type convert_construct(
          T&& operand
        , int
        , mpl::false_ = mpl::false_() // is_foreign_variant
        )
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename qsboost::enable_if<qsboost::is_rvalue_reference<Variant&&> >::type convert_construct(
          Variant&& operand
        , long
        , mpl::true_// is_foreign_variant
        )
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES
    template <typename Variant>
    typename qsboost::enable_if<qsboost::is_rvalue_reference<Variant&&> >::type convert_construct_variant(Variant&& operand)
    {
        // [Determine if the given variant is itself a bounded type, or if its
        //  content needs to be converted (i.e., it is a 'foreign' variant):]
        //
    ...
    template <QSBOOST_VARIANT_ENUM_PARAMS(typename U)>
    void convert_construct(
          qsboost::variant<QSBOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
    ...
    template <QSBOOST_VARIANT_ENUM_PARAMS(typename U)>
    void convert_construct(
          const qsboost::variant<QSBOOST_VARIANT_ENUM_PARAMS(U)>& operand
        , long
        )
    {
        convert_construct_variant(operand);    
...
#if !defined(QSBOOST_VARIANT_AUX_BROKEN_CONSTRUCTOR_TEMPLATE_ORDERING)
...
#elif defined(QSBOOST_VARIANT_AUX_HAS_CONSTRUCTOR_TEMPLATE_ORDERING_SFINAE_WKND)
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    variant(T&& operand, typename qsboost::enable_if<qsboost::is_rvalue_reference<T&&> >::type* = 0, 
        typename qsboost::disable_if<qsboost::is_const<T> >::type* = 0)
    {
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES
    variant(variant&& operand) QSBOOST_NOEXCEPT_IF(variant_move_noexcept_constructible::type::value)
    {
...
#   endif
    ...
    public: // structors
        ...
        assigner(variant& lhs, int rhs_which) QSBOOST_NOEXCEPT
            : lhs_(lhs)
            , rhs_which_(rhs_which)
        {
        }
    ...
    protected: // helpers, for internal visitor interface (below)
        ...
        void construct_fallback() const QSBOOST_NOEXCEPT {
            ...
            lhs_.indicate_which(
                  QSBOOST_MPL_AUX_VALUE_WKND(fallback_type_index_)::value
                ); // nothrow
        }
    ...
    public: // structors
        ...
        move_assigner(variant& lhs, int rhs_which) QSBOOST_NOEXCEPT
...
public: // modifiers
...
#ifndef QSBOOST_NO_CXX11_RVALUE_REFERENCES
    template <class T>
    typename qsboost::enable_if_c<qsboost::is_rvalue_reference<T&&>::value && !qsboost::is_const<T>::value, variant& >::type 
        operator=(T&& rhs) 
    {
    ...
    variant& operator=(variant&& rhs) 
        ...
        QSBOOST_NOEXCEPT_IF(variant_move_noexcept_constructible::type::value && variant_move_noexcept_assignable::type::value)
#endif
    {
...
public: // queries
    ...
    const qsboost::typeindex::type_info& type() const
    {
...
// Obsolete. Remove.
#   define QSBOOST_VARIANT_AUX_FAIL_COMPARISON_RETURN_TYPE \
    void

    template <typename U>
    void operator==(const U&) const
    {
        QSBOOST_STATIC_ASSERT( false && sizeof(U) );
    }
...
inline void swap(
      variant< QSBOOST_VARIANT_ENUM_PARAMS(T) >& lhs
    , variant< QSBOOST_VARIANT_ENUM_PARAMS(T) >& rhs
    )
{
    lhs.swap(rhs);
}
...
#include "qsboost/variant/detail/variant_io.hpp"
