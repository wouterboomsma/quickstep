...
#ifndef QSBOOST_VARIANT_DETAIL_FORCED_RETURN_HPP
#define QSBOOST_VARIANT_DETAIL_FORCED_RETURN_HPP
...
//
 ...
 && !defined(QSBOOST_NO_VOID_RETURNS)
...
template <typename T>
inline
    QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(T)
forced_return()
{
    // logical error: should never be here! (see above)
    QSBOOST_ASSERT(false);

    QSBOOST_VARIANT_AUX_GENERIC_RESULT_TYPE(T) (*dummy)() = 0;
    return dummy();
}
