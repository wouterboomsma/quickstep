...
#ifndef QSBOOST_TYPEOF_MSVC_TYPEOF_IMPL_HPP_INCLUDED
# define QSBOOST_TYPEOF_MSVC_TYPEOF_IMPL_HPP_INCLUDED
...
# if QSBOOST_WORKAROUND(QSBOOST_MSVC,>=1300) && defined(_MSC_EXTENSIONS)
        ...
        struct encode_counter
        {
            ...
            {
                QSBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 257>::count));
            }
            __if_not_exists(the_counter<N + 256>)
            {
                __if_exists(the_counter<N + 64>)
                {
                    QSBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 65>::count));
                }
                __if_not_exists(the_counter<N + 64>)
                {
                    __if_exists(the_counter<N + 16>)
                    {
                        QSBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 17>::count));
                    }
                    __if_not_exists(the_counter<N + 16>)
                    {
                        __if_exists(the_counter<N + 4>)
                        {
                            QSBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 5>::count));
                        }
                        __if_not_exists(the_counter<N + 4>)
                        {
                            __if_exists(the_counter<N>)
                            {
                                QSBOOST_STATIC_CONSTANT(unsigned,count=(encode_counter<T,N + 1>::count));
                            }
                            __if_not_exists(the_counter<N>)
                            {
                                QSBOOST_STATIC_CONSTANT(unsigned,count=N);
...
# define QSBOOST_TYPEOF_INDEX(T) (encode_counter<T>::count)
# define QSBOOST_TYPEOF_NEXT_INDEX(next)
# else
...
# define QSBOOST_TYPEOF_INDEX(T) (sizeof(*qsboost::type_of::encode_index((qsboost::type_of::encode_counter<1005>*)0)))
# define QSBOOST_TYPEOF_NEXT_INDEX(next) friend char (*encode_index(encode_counter<next>*))[next];
# endif
...
# define QSBOOST_TYPEOF(expr) \
    qsboost::type_of::msvc_typeid_wrapper<typeid(qsboost::type_of::encode_start(expr))>::type

# define QSBOOST_TYPEOF_TPL(expr) typename QSBOOST_TYPEOF(expr)

# define QSBOOST_TYPEOF_NESTED_TYPEDEF_TPL(name,expr) \
struct name {\
    enum {_typeof_register_value=sizeof(typeid(qsboost::type_of::typeof_register_type<name>(expr)))};\
    typedef typename qsboost::type_of::msvc_extract_type<name>::id2type id2type;\
...
struct name {\
    enum {_typeof_register_value=sizeof(typeid(qsboost::type_of::typeof_register_type<name>(expr)))};\
    typedef qsboost::type_of::msvc_extract_type<name>::id2type id2type;\
...
# define QSBOOST_TYPEOF(expr) \
    qsboost::type_of::msvc_typeid_wrapper<sizeof(*qsboost::type_of::encode_start(expr))>::type

# define QSBOOST_TYPEOF_TPL(expr) typename QSBOOST_TYPEOF(expr)

# define QSBOOST_TYPEOF_NESTED_TYPEDEF_TPL(name,expr) \
    struct name {\
        QSBOOST_STATIC_CONSTANT(int,_typeof_register_value=sizeof(qsboost::type_of::typeof_register_type<name>(expr)));\
        typedef typename qsboost::type_of::msvc_extract_type<name>::id2type id2type;\
        typedef typename id2type::type type;\
    };
