...
#ifndef QSBOOST_TT_IS_BASE_AND_DERIVED_HPP_INCLUDED
#define QSBOOST_TT_IS_BASE_AND_DERIVED_HPP_INCLUDED

#include <qsboost/type_traits/intrinsics.hpp>
#include <qsboost/type_traits/integral_constant.hpp>
#ifndef QSBOOST_IS_BASE_OF
...
namespace detail {
 ...
 && !QSBOOST_WORKAROUND(__SUNPRO_CC , <= 0x540) \
...
#pragma warning(disable:6334)
    ...
    //
    QSBOOST_STATIC_ASSERT(sizeof(B) != 0);
    QSBOOST_STATIC_ASSERT(sizeof(D) != 0);
...
{
    QSBOOST_STATIC_CONSTANT(bool, value =
        (::qsboost::is_convertible<D*,B*>::value));
};

#define QSBOOST_BROKEN_IS_BASE_AND_DERIVED
...
struct is_base_and_derived_impl
{
    ...
    typedef is_base_and_derived_select<
       ::qsboost::is_class<B>::value,
       ::qsboost::is_class<D>::value,
       ::qsboost::is_same<ncvB,ncvD>::value> selector;
    typedef typename selector::template rebind<ncvB,ncvD> binder;
    typedef typename binder::type bound_type;

    QSBOOST_STATIC_CONSTANT(bool, value = bound_type::value);
};
#else
template <typename B, typename D>
struct is_base_and_derived_impl
{
    ...
    QSBOOST_STATIC_CONSTANT(bool, value = (QSBOOST_IS_BASE_OF(B,D) && ! ::qsboost::is_same<ncvB,ncvD>::value));
};
#endif
} // namespace detail

template <class Base, class Derived> struct is_base_and_derived
   : public integral_constant<bool, (::qsboost::detail::is_base_and_derived_impl<Base, Derived>::value)> {};

template <class Base, class Derived> struct is_base_and_derived<Base&, Derived> : public false_type{};
template <class Base, class Derived> struct is_base_and_derived<Base, Derived&> : public false_type{};
template <class Base, class Derived> struct is_base_and_derived<Base&, Derived&> : public false_type{};

#if QSBOOST_WORKAROUND(__CODEGEARC__, QSBOOST_TESTED_AT(0x610))
