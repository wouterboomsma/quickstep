#ifndef QSBOOST_TT_IS_ABSTRACT_CLASS_HPP
#define QSBOOST_TT_IS_ABSTRACT_CLASS_HPP
...
#include <qsboost/type_traits/is_polymorphic.hpp>
#endif
#endif
...
#ifdef QSBOOST_IS_ABSTRACT
template <class T>
struct is_abstract_imp
{
   QSBOOST_STATIC_CONSTANT(bool, value = QSBOOST_IS_ABSTRACT(T));
};
...
struct is_abstract_imp2
{
   ...
   //
   QSBOOST_STATIC_ASSERT(sizeof(T) != 0);

   // GCC2 won't even parse this template if we embed the computation
   // of s1 in the computation of value.
#ifdef __GNUC__
   QSBOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(is_abstract_imp2<T>::template check_sig<T>(0)));
#else
#if QSBOOST_WORKAROUND(QSBOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(push)
#pragma warning(disable:6334)
#endif
   QSBOOST_STATIC_CONSTANT(std::size_t, s1 = sizeof(check_sig<T>(0)));
#if QSBOOST_WORKAROUND(QSBOOST_MSVC_FULL_VER, >= 140050000)
#pragma warning(pop)
#endif
#endif
...
struct is_abstract_imp
{
   typedef is_abstract_select< ::qsboost::is_class<T>::value> selector;
...
#ifndef QSBOOST_NO_IS_ABSTRACT
template <class T> struct is_abstract : public integral_constant<bool, ::qsboost::detail::is_abstract_imp<T>::value> {};
#else
template <class T> struct is_abstract : public integral_constant<bool, ::qsboost::detail::is_polymorphic_imp<T>::value> {};
#endif
