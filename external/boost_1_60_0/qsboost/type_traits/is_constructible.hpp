...
#ifndef QSBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED
#define QSBOOST_TT_IS_CONSTRUCTIBLE_HPP_INCLUDED

#include <qsboost/type_traits/integral_constant.hpp>
#include <qsboost/detail/workaround.hpp>

#if !defined(QSBOOST_NO_CXX11_VARIADIC_TEMPLATES) && !defined(QSBOOST_NO_CXX11_DECLTYPE) && !QSBOOST_WORKAROUND(QSBOOST_MSVC, < 1800) && !QSBOOST_WORKAROUND(QSBOOST_GCC_VERSION, < 40500)
...
namespace qsboost{
   ...
   namespace detail{
      ...
      {
         template<typename T, typename ...TheArgs, typename = decltype(T(qsboost::declval<TheArgs>()...))>
         static qsboost::type_traits::yes_type test(int);
         template<typename, typename...>
         static qsboost::type_traits::no_type test(...);

         template<typename T, typename Arg, typename = decltype(::new T(qsboost::declval<Arg>()))>
         static qsboost::type_traits::yes_type test1(int);
         template<typename, typename>
         static qsboost::type_traits::no_type test1(...);

         template <typename T>
         static qsboost::type_traits::yes_type ref_test(T);
         template <typename T>
         static qsboost::type_traits::no_type ref_test(...);
      };

   }

   template <class T, class ...Args> struct is_constructible : public integral_constant<bool, sizeof(detail::is_constructible_imp::test<T, Args...>(0)) == sizeof(qsboost::type_traits::yes_type)>{};
   template <class T, class Arg> struct is_constructible<T, Arg> : public integral_constant<bool, is_destructible<T>::value && sizeof(detail::is_constructible_imp::test1<T, Arg>(0)) == sizeof(qsboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&>(qsboost::declval<Arg>())) == sizeof(qsboost::type_traits::yes_type)>{};
   template <class Ref, class Arg> struct is_constructible<Ref&&, Arg> : public integral_constant<bool, sizeof(detail::is_constructible_imp::ref_test<Ref&&>(qsboost::declval<Arg>())) == sizeof(qsboost::type_traits::yes_type)>{};
