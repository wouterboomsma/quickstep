...
#ifndef QSBOOST_TT_HAS_NOTHROW_ASSIGN_HPP_INCLUDED
#define QSBOOST_TT_HAS_NOTHROW_ASSIGN_HPP_INCLUDED

#include <qsboost/type_traits/integral_constant.hpp>
#include <qsboost/type_traits/intrinsics.hpp>

#if !defined(QSBOOST_HAS_NOTHROW_ASSIGN) || defined(QSBOOST_MSVC) || defined(QSBOOST_INTEL)
#include <qsboost/type_traits/has_trivial_assign.hpp>
#if !defined(QSBOOST_NO_CXX11_NOEXCEPT) && !defined(QSBOOST_NO_CXX11_RVALUE_REFERENCES)
...
namespace qsboost {
...
#if !defined(QSBOOST_HAS_NOTHROW_ASSIGN) && !defined(QSBOOST_NO_CXX11_NOEXCEPT) && !defined(QSBOOST_NO_CXX11_RVALUE_REFERENCES)

   namespace detail
   {
      template <class T, bool b1, bool b2> struct has_nothrow_assign_imp{ static const bool value = false; };
      template <class T>          struct has_nothrow_assign_imp<T, false, true>{ static const bool value = noexcept(qsboost::declval<typename add_reference<T>::type>() = qsboost::declval<typename add_reference<T const>::type>()); };
   ...
   struct has_nothrow_assign : public integral_constant < bool,
#ifndef QSBOOST_HAS_NOTHROW_ASSIGN
#if !defined(QSBOOST_NO_CXX11_NOEXCEPT) && !defined(QSBOOST_NO_CXX11_RVALUE_REFERENCES)
      ...
      >::value
#else
      ::qsboost::has_trivial_assign<T>::value
#endif
#else
      QSBOOST_HAS_NOTHROW_ASSIGN(T)
#endif
   > {};
