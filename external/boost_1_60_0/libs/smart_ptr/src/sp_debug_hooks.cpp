...
void sp_scalar_constructor_hook(void * p)
{
    ...
    QSBOOST_ASSERT(*pm != adopted_scalar);    // second smart pointer to the same address
    QSBOOST_ASSERT(*pm != allocated_array);   // allocated with new[]
    QSBOOST_ASSERT(*pm == allocated_scalar);  // not allocated with new
...
void sp_scalar_destructor_hook(void * p)
{
    ...
    QSBOOST_ASSERT(*pm == adopted_scalar);    // attempt to destroy nonmanaged block
...
void operator delete(void * p) throw()
{
    ...
    QSBOOST_ASSERT(*pm != deleted);           // double delete
    QSBOOST_ASSERT(*pm != adopted_scalar);    // delete p.get();
    QSBOOST_ASSERT(*pm != allocated_array);   // allocated with new[]
    QSBOOST_ASSERT(*pm == allocated_scalar);  // not allocated with new
...
void operator delete[](void * p) throw()
{
    ...
    QSBOOST_ASSERT(*pm != deleted);           // double delete
    QSBOOST_ASSERT(*pm != adopted_scalar);    // delete p.get();
    QSBOOST_ASSERT(*pm != allocated_scalar);  // allocated with new
    QSBOOST_ASSERT(*pm == allocated_array);   // not allocated with new[]
