...
public:
    ...
    inline Real angle(const Vector3& a) const;
...
public:
    ...
    inline void setRotate(const Vector3& axis, Real rads);  ///<sets the matrix that rotates ccw by rads around axis
...
public:
    ...
    inline void setRotate(const Vector3& axis, Real rads);
...
inline Real Vector3::angle(const Vector3& a) const { return Acos(dot(a)/(norm()*a.norm())); }
...
inline void Matrix3::setRotate(const Vector3& axis, Real rads)
{
    double len = Sqrt(axis.x*axis.x + axis.y*axis.y + axis.z*axis.z);
    Vector3 scaledAxis(axis);scaledAxis/=len;
    //scaledAxis.inplaceNormalize();
    Real c = Cos(rads);
    Real s = Sin(rads);
    Real omc = 1 - c;
...
inline void RigidTransform::setRotate(const Vector3& axis, Real rads){ R.setRotate(axis, rads); t.setZero(); }
