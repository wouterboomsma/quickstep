...
KinematicForest::KinematicForest(quickstep::Topology &topology):
	...
	//adjacency_list.resize(coordinates.cols());
...
void KinematicForest::initialize(const Platform &platform, const CoordinatesWrapper &coordinates) {
    ...
    for( auto bond: topology->get_bonds() ){
    	...
    	if(!ds.connected(a1.index, a2.index)){
            adjacency_list[a1.index].push_back( pair<int,int>(a1.index, a2.index) );
            adjacency_list[a2.index].push_back( pair<int,int>(a1.index, a2.index) );
    ...
    kernel.get_as<KinematicForestKernel>().initialize(*this, adjacency_list, roots);
...
//}
//
units::Length KinematicForest::get_length(int atom_index) {
    return kernel.get_as<KinematicForestKernel>().get_length(atom_index) * units::nm;
}

void KinematicForest::increment_length(int atom_index, units::Length value) {
    kernel.get_as<KinematicForestKernel>().increment_length(atom_index, units::Length_nm(value).value());
}
...
void KinematicForest::modify_dof(Dof &dof, double delta_value) {
    modified_dofs.push_back(std::make_pair(std::ref(dof), delta_value));
}
...
void KinematicForest::update_positions() {
    ...
    //}
    //
    //kernel.get_as<KinematicForestKernel>().forward_propagate_transformations(moved_subtrees);
    //moved_subtrees.clear();

    //for(const int& subtree_root: moved_subtrees){
    //    //cout<<"Moving subtree "<<subtree_root<<endl;
    //    kernel.get_as<KinematicForestKernel>().forward_propagate_transformations(subtree_root);
    //}
...
void KinematicForest::restore_positions() {
    kernel.get_as<KinematicForestKernel>().restore_positions();
...
int KinematicForest::get_parent(int v) const {
    return kernel.get_as<KinematicForestKernel>().get_parent(v);
}
...
//
//
bool KinematicForest::atom_matches_names(int atom, const std::vector<std::string>& dofNames) const {
	...
	for (int p=0;p<dofNames.size();p++) {
		if(a<0 || a>=n_atoms || topology->get_atoms()[a].name!=dofNames[p]) {
            matches = false;
            break;
        }
		a = get_parent(a);
    ...
    for (int p=dofNames.size()-1; p>=0; --p) {
        ...
        if(a < 0 || topology->get_atoms()[a].name != dofNames[p]) {
            matches = false;
            break;
        }
        a = get_parent(a);
...
std::unique_ptr<Dof> KinematicForest::construct_dof(const vector<int> &atom_indices, const vector<string> &atom_names) const {
    if (!atom_names.empty() && !atom_matches_names(atom_indices.back(), atom_names)) {
    ...
    switch (atom_indices.size()) {
        case 2: {
            int atom_index = ((get_parent(atom_indices[0]) == atom_indices[1]) ?
                              atom_indices[0] : atom_indices[1]);
            return std::make_unique<LengthDof>(get_ancestors(atom_index, Dof::LENGTH));
        }
        case 3: {
            int atom_index = ((get_parent(atom_indices[0]) == atom_indices[1]) ?
                              atom_indices[0] : atom_indices[2]);
            return std::make_unique<AngleDof>(get_ancestors(atom_index, Dof::ANGLE));
        }
        case 4: {
            int atom_index = ((get_parent(atom_indices[0]) == atom_indices[1]) ?
                              atom_indices[0] : atom_indices[3]);
            //Collect all children of parent
            int parent_index = get_parent(atom_index);
            return std::make_unique<DihedralDof>(get_ancestors(atom_index, Dof::DIHEDRAL),
                                                 get_children(parent_index));
        }
        default:
            QSBOOST_THROW_EXCEPTION(FatalError() <<
...
//}
//
std::vector<int> KinematicForest::get_ancestors(int atom_index, Dof::Type dof_type) const {
    std::vector<int> atoms(dof_type+1);
    atoms[0] = atom_index;
    int current_atom_index = atom_index;
    for (int i=1; i<dof_type+1; ++i) {
        current_atom_index = get_parent(current_atom_index);
        atoms[i] = current_atom_index;
    }
    return atoms;
}
...
std::vector<int> KinematicForest::get_children(int parent_index) const {
