...
void Topology::create_disulfide_bonds(const std::vector<std::array<units::Length_AA, 3> > &positions) {
    ...
    for (int i=0; i<cyx.size(); ++i) {
        ...
        const std::array<units::Length_AA, 3> &pos1 = positions[sg1.index];
        for (int j=0; j<i; ++j) {
            const Atom &sg2 = cyx_atoms_by_name[j].at("SG");
            const std::array<units::Length_AA, 3> &pos2 = positions[sg2.index];
            units::Length_AA distance = boost::units::root<2>(boost::units::pow<2>(pos2[0] - pos1[0]) +
